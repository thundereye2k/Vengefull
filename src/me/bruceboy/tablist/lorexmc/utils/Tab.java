package me.bruceboy.tablist.lorexmc.utils;

import lombok.Getter;
import lombok.Setter;
import me.bruceboy.tablist.lorexmc.TabList;
...
public class Tab {
	...
	public Tab(Player player, boolean hook, TabList tabList) {
		...
		for (Player other : Bukkit.getOnlinePlayers()) {
			...
			getElevatedTeam(other, tabList).addEntry(other.getName());

			Tab tab = tabList.getTabByPlayer(other);
			if (tab != null) {
				tab.getElevatedTeam(player, tabList).addEntry(player.getName());
			}
	...
	public Team getElevatedTeam(Player player, TabList tabList) {
			...
			for (int x = 0; x < 4; x++) {
				for (int y = 0; y < 20; y++) {
					...
					((CraftPlayer) player).getHandle().playerConnection
							.sendPacket(getPlayerPacket(entries.get(position)));
