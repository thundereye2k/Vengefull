...
public class EotwListener implements Listener {
	...
	public EotwListener(LorexHCF plugin) {
		this.plugin = plugin;
	}
	 ...
	 * void onFactionCreate(FactionCreateEvent event) { if
	 * (plugin.getEotwHandler().isEndOfTheWorld()) { FactionListener faction =
	 * event.getFaction(); if (faction instanceof PlayerFaction) {
	 * event.setCancelled(true); event.getSender().sendMessage(ChatColor.RED +
	 * "Player based factions cannot be created during EOTW."); } } }
	 */
