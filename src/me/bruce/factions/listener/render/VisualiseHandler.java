package me.bruce.factions.listener.render;
...
public class VisualiseHandler {
	...
	public LinkedHashMap<Location, VisualBlockData> generateAsync(Player player, Cuboid cuboid, VisualType visualType,
			boolean canOverwrite) {
		Collection<Location> locations = new HashSet<>(cuboid.getSizeX() * cuboid.getSizeY() * cuboid.getSizeZ());
		Iterator<Location> iterator = cuboid.locationIterator();
		while (iterator.hasNext()) {
	...
	public LinkedHashMap<Location, VisualBlockData> generateAsync(Player player, Iterable<Location> locations,
			VisualType visualType, boolean canOverwrite) {
		synchronized (storedVisualises) {
			LinkedHashMap<Location, VisualBlockData> results = new LinkedHashMap<>();
				...
				for (Location location : locations) {
					...
					location.getWorld().getChunkAtAsync(location, new World.ChunkLoadCallback() {
						...
						public void onLoad(Chunk chunk) {
							Material previousType = CraftMagicNumbers.getMaterial(((CraftChunk) chunk).getHandle()
									.getType(location.getBlockX(), location.getBlockY(), location.getBlockZ()));
							if (previousType.isSolid() || previousType != Material.AIR) {
								return;
							}
	 ...
	 *            if a packet to send a block change should be sent (this is used to
	 *            prevent unnecessary packets sent when disconnecting or changing
	 *            worlds, for example)
	 * @return if the visual block was shown in the first place
	 */
