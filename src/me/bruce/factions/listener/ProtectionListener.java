...
public class ProtectionListener implements Listener {
	...
	private static final ImmutableSet<Material> BLOCK_INTERACTABLES = Sets.immutableEnumSet(Material.BED,
			...
			Material.DISPENSER, Material.STONE_BUTTON, Material.WOOD_BUTTON, Material.ENCHANTMENT_TABLE, Material.ANVIL,
			Material.LEVER, Material.FIRE);

	private final LorexHCF plugin;

	public ProtectionListener(LorexHCF plugin) {
		this.plugin = plugin;
	}
	...
	public void onStickyPistonExtend(BlockPistonExtendEvent event) {
		...
		if (targetBlock.isEmpty() || targetBlock.isLiquid()) { // If potentially pushing into AIR/WATER/LAVA in another
			...
			if (targetFaction instanceof Raidable && !((Raidable) targetFaction).isRaidable()
					&& targetFaction != plugin.getFactionManager().getFactionAt(block)) {
				event.setCancelled(true);
			}
		}
	}
	...
	public void onBlockFromTo(BlockFromToEvent event) {
		...
		if (SettingsYML.DISABLE_OBSIDIAN_GENERATORS) {
	...
	public void onPlayerTeleport(PlayerTeleportEvent event) {
		if (event.getCause() == PlayerTeleportEvent.TeleportCause.ENDER_PEARL) {
			...
			if (toFactionAt.isSafezone() && !plugin.getFactionManager().getFactionAt(event.getFrom()).isSafezone()) {
				Player player = event.getPlayer();
				player.sendMessage(
						ChatColor.RED + "You may not Enderpearl into safe-zones, used Enderpearl has been refunded.");
		...
		if (entity instanceof Player) {
			...
			if (attacker != null) {
				...
				if (playerFaction != null
						&& ((attackerFaction = plugin.getFactionManager().getPlayerFaction(attacker)) != null)) {
					...
					if (attackerFaction == playerFaction) {
						...
						if (true) { // TODO: prevent ally damage.
							...
							attacker.sendMessage(
									SettingsYML.ALLY_COLOUR + hiddenAstrixedName + ChatColor.YELLOW + " is an ally.");
						} else {
							attacker.sendMessage(ChatColor.YELLOW + "Careful! " + SettingsYML.ALLY_COLOUR
		...
		if (entered instanceof Player) {
			...
			if (vehicle instanceof Horse) {
				...
				if (owner != null && !owner.equals(entered)) {
					((Player) entered).sendMessage(ChatColor.YELLOW + "You cannot ride a Horse that belongs to "
	...
	public void onFoodLevelChange(FoodLevelChangeEvent event) {
		...
		if (entity instanceof Player && ((Player) entity).getFoodLevel() > event.getFoodLevel()
				&& plugin.getFactionManager().getFactionAt(entity.getLocation()).isSafezone()) {
			event.setCancelled(true);
		}
	}
		...
		if (source instanceof Player) {
			...
			for (LivingEntity affected : event.getAffectedEntities()) {
				if (affected instanceof Player && !player.equals(affected)) {
					...
					if (plugin.getFactionManager().getFactionAt(target.getLocation())
	...
	public void onEntityTarget(EntityTargetEvent event) {
		...
		case RANDOM_TARGET:
			...
			if (event.getEntity() instanceof LivingEntity && target instanceof Player) {
				...
				if (factionAt.isSafezone()
						|| ((playerFaction = plugin.getFactionManager().getPlayerFaction((Player) target)) != null
								&& factionAt == playerFaction)) {
					event.setCancelled(true);
				}
			}
			break;
	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		...
		if (action == Action.PHYSICAL) { // Prevent players from trampling on crops or pressure plates, etc.
			...
			if (canBuild) {
				...
				if (itemType != null && ITEM_BLOCK_INTERACTABLES.containsKey(itemType)
					...
					if (block.getType() != Material.WORKBENCH
							|| !event.getPlayer().hasPermission("utilities.openchestsilent")
							|| !plugin.getFactionManager().getFactionAt(block).isSafezone()) {
						canBuild = false;
					}
				} else {
			...
			if (!canBuild && !attemptBuild(event.getPlayer(), block.getLocation(),
					ChatColor.YELLOW + "You cannot do this in the territory of %1$s" + ChatColor.YELLOW + '.', true)) {
				event.setCancelled(true);
			}
		}
	}
	 ...
	 * plugin.getFactionManager().getFactionAt(event.getBlock().getLocation()); if
	 * (factionAt instanceof ClaimableFaction && !(factionAt instanceof
	 * PlayerFaction)) { event.setCancelled(true); } }
	 */
	...
	public void onBlockBreak(BlockBreakEvent event) {
		if (LorexHCF.getInstance().getFactionManager().getFactionAt(event.getPlayer().getLocation()) == LorexHCF
				.getInstance().getFactionManager().getFaction("GlowstoneMountain")) {
			if (event.getBlock().getType().equals(Material.GLOWSTONE)) {
				return;
			}
		}
		if (!attemptBuild(event.getPlayer(), event.getBlock().getLocation(),
				ChatColor.YELLOW + "You may not build in the territory of %1$s" + ChatColor.YELLOW + '.')) {
			event.setCancelled(true);
		}
	}
	...
	public static boolean attemptBuild(Entity entity, Location location, @Nullable String denyMessage,
		...
		Faction factionAt = LorexHCF.getInstance().getFactionManager().getFactionAt(location);
		if (!(factionAt instanceof ClaimableFaction)) {
			result = true;
		...
		if (player != null && factionAt instanceof PlayerFaction) {
			if (LorexHCF.getInstance().getFactionManager().getPlayerFaction(player) == factionAt) {
				result = true;
			}
		}
		...
		if (result) {
			...
			if (!isInteraction && factionAt instanceof WarzoneFaction) {
				if (denyMessage != null && player != null) {
					player.sendMessage(ChatColor.YELLOW + "You may not build in the " + factionAt.getDisplayName(player)
							+ ChatColor.YELLOW + ".");
				}
	...
	public static boolean canBuildAt(Location from, Location to) {
		Faction toFactionAt = LorexHCF.getInstance().getFactionManager().getFactionAt(to);
		return !(toFactionAt instanceof Raidable && !((Raidable) toFactionAt).isRaidable()
				&& toFactionAt != LorexHCF.getInstance().getFactionManager().getFactionAt(from));
	}
}
