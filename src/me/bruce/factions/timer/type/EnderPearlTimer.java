...
public class EnderPearlTimer extends PlayerTimer implements Listener {
	...
	public void onExpire(UUID userUUID) {
		...
		if (player != null) {
			player.sendMessage(ChatColor.GREEN + "Your " + getDisplayName() + ChatColor.GREEN
					+ " timer has expired. You may now Enderpearl again.");
		}
	}
	...
	public void onProjectileLaunch(ProjectileLaunchEvent event) {
		...
		if (projectile instanceof EnderPearl) {
			...
			if (source instanceof Player) {
				...
				if (remaining > 0L) {
					shooter.sendMessage(ChatColor.RED + "You are still on " + ChatColor.DARK_RED + getName()
							+ ChatColor.RED + " cooldown for another " + ChatColor.BOLD
	...
	public void startDisplaying(Player player) {
		...
		if (getRemaining(player) > 0L && itemNameFakes.putIfAbsent(player.getUniqueId(),
			...
			long ticks = ((CraftPlayer) player).getHandle().playerConnection.networkManager.getVersion() >= 47
	...
	public static class PearlNameFaker extends BukkitRunnable {
		...
		public void run() {
			...
			if (stack != null && stack.getType() == Material.ENDER_PEARL) {
				...
				if (remaining > 0L) {
					...
					item.c(ChatColor.YELLOW + "Enderpearl Cooldown" + ChatColor.GOLD + " » " + ChatColor.GRAY
		...
		public synchronized void cancel() throws IllegalStateException {
			super.cancel();
			setFakeItem(CraftItemStack.asNMSCopy(player.getItemInHand()), player.getInventory().getHeldItemSlot()); // show
																													// the
																													// original
																													// item
																													// here.
