package me.bruce.factions.faction.claim;

import com.google.common.base.Preconditions;

import LorexMC.us.utils.ItemBuilder;
import LorexMC.us.utils.other.cuboid.Cuboid;
import LorexMC.us.utils.other.cuboid.CuboidDirection;
...
import me.bruce.factions.faction.type.*;
...
public class ClaimHandler {
	...
	public static final ItemStack SUBCLAIM_WAND = new ItemBuilder(Material.GOLD_SPADE, 1)
			...
			.lore(ChatColor.AQUA + "Left or Right Click " + ChatColor.GREEN + "a Block" + ChatColor.AQUA + " to:",
					...
					ChatColor.GRAY + "Clear your current Subclaim selection.", "", ChatColor.AQUA + "Use "
							+ ChatColor.YELLOW + "/faction subclaim create <name>" + ChatColor.AQUA + " to:",
					ChatColor.GRAY + "Acquire your selected Subclaim.")
			.build();

	public static final ItemStack CLAIM_WAND = new ItemBuilder(Material.WOOD_HOE)
			.displayName(ChatColor.RED + "Claim Wand")
			.lore(ChatColor.AQUA + "Left or Right Click " + ChatColor.GREEN + "a Block" + ChatColor.AQUA + " to:",
					ChatColor.GRAY + "Set the first and second position of ", ChatColor.GRAY + "your Claim selection.",
					"", ChatColor.AQUA + "Right Click " + ChatColor.GREEN + "the Air" + ChatColor.AQUA + " to:",
					ChatColor.GRAY + "Clear your current Claim selection.", "",
					ChatColor.YELLOW + "Shift " + ChatColor.AQUA + "Left Click " + ChatColor.GREEN
							+ "the Air or a Block" + ChatColor.AQUA + " to:",
					ChatColor.GRAY + "Purchase your current Claim selection.")
			.build();

	private static final int NEXT_PRICE_MULTIPLIER_AREA = 250; // the area a claim cuboid needs until the price
																// multiplier is increased
	private static final int NEXT_PRICE_MULTIPLIER_CLAIM = 500; // the amount each claim a faction has will add onto the
																// final price
	...
	public ClaimHandler(LorexHCF plugin) {
		this.plugin = plugin;
	...
	/**
	 * Gets the price of this {@link Claim} for a given {@link FactionListener}.
	 *
	 * @param claim
	 ...
	 * @param selling
	 *            if the {@link FactionListener} is selling the claim
	...
	/**
	 * Checks if a {@link Player} is eligible to {@link Subclaim} at a given
	 * {@link Location}.
	 *
	 * @param player
	 ...
	 *
	 * @param player
	 *            the {@link Player} that is attempting to create the
	 *            {@link Subclaim}
	 * @param subclaim
	...
	public boolean tryCreatingSubclaim(Player player, Subclaim subclaim) {
		...
		for (int x = minimumX; x < maximumX; x++) {
			for (int z = minimumZ; z < maximumZ; z++) {
				...
				if (claimAt == null
						|| playerFaction == (factionAt = plugin.getFactionManager().getFactionAt(world, x, z))
								&& !(factionAt instanceof PlayerFaction)) {
					player.sendMessage(
							ChatColor.RED + "This subclaim selection contains a location outside of your faction.");
					return false;
				}
				...
				if (foundClaim == null) {
					...
					player.sendMessage(
							ChatColor.RED + "This subclaim selection is inside more than one of your faction claims.");
					return false;
				}
			}
		}
		...
		player.sendMessage(ChatColor.GOLD + "You have created a subclaim named " + ChatColor.AQUA + subclaim.getName()
				+ ChatColor.GOLD + '.');
		return true;
	}

	/**
	 * Checks if a {@link Player} is eligible to {@link Claim} at a given
	 * {@link Location}.
	 *
	 * @param player
	 *            the {@link Player} to check for
	...
	public boolean canClaimHere(Player player, Location location) {
		...
		if (!(plugin.getFactionManager().getFactionAt(location) instanceof WildernessFaction)) {
			player.sendMessage(ChatColor.RED + "You can only claim land in the " + SettingsYML.WILDERNESS_COLOUR
					+ "Wilderness" + ChatColor.RED + ". " + "Make sure you are past " + SettingsYML.WARZONE_RADIUS
					+ " blocks from spawn..");
		...
		if (playerFaction.getClaims().size() >= SettingsYML.MAX_CLAIMS_PER_FACTION) {
			player.sendMessage(
					ChatColor.RED + "Your faction has maximum claims - " + SettingsYML.MAX_CLAIMS_PER_FACTION);
			return false;
		}
		...
		for (int x = locX - CLAIM_BUFFER_RADIUS; x < locX + CLAIM_BUFFER_RADIUS; x++) {
			for (int z = locZ - CLAIM_BUFFER_RADIUS; z < locZ + CLAIM_BUFFER_RADIUS; z++) {
				...
				if (!SettingsYML.ALLOW_CLAIMING_BESIDES_ROADS && factionAtNew instanceof ClaimableFaction
						&& playerFaction != factionAtNew && !(factionAtNew instanceof RoadFaction)) {
					player.sendMessage(ChatColor.RED + "This position contains enemy claims within a "
							+ CLAIM_BUFFER_RADIUS + " block buffer radius.");
					return false;
				}
			}
		}
	...
	/**
	 * Tries to purchase a {@link Claim} for a {@link Player}s
	 * {@link PlayerFaction}'.
	 *
	 * @param player
	 *            the {@link Player} that is attempting to create the
	...
	public boolean tryPurchasing(Player player, Claim claim) {
		...
		if (world.getEnvironment() != World.Environment.NORMAL) {
			player.sendMessage(ChatColor.RED + "You can only claim in the OVERWORLD.");
			return false;
		}
		...
		if (playerFaction == null) {
			player.sendMessage(ChatColor.RED + "You must be in a faction to claim.");
			return false;
		}

		if (playerFaction.getMember(player.getUniqueId()).getRole() == Role.MEMBER) {
			player.sendMessage(ChatColor.RED + "You must be an officer to claim.");
			return false;
		}
		...
		if (claimPrice > factionBalance) {
			player.sendMessage(ChatColor.RED + "You have " + EconomyManager.ECONOMY_SYMBOL + factionBalance
					+ ", The price of this claim is " + EconomyManager.ECONOMY_SYMBOL + claimPrice + '.');

			return false;
		}

		if (claim.getChunks().size() > MAX_CHUNKS_PER_LIMIT) {
			player.sendMessage(ChatColor.RED + "The max chunks are " + MAX_CHUNKS_PER_LIMIT + " chunks.");
			return false;
		}
		...
		if (claim.getWidth() < MIN_CLAIM_RADIUS || claim.getLength() < MIN_CLAIM_RADIUS) {
			player.sendMessage(
					ChatColor.RED + "Claims must be atleast " + MIN_CLAIM_RADIUS + 'x' + MIN_CLAIM_RADIUS + " blocks.");
			return false;
		}
		...
		for (int x = minimumX; x < maximumX; x++) {
			for (int z = minimumZ; z < maximumZ; z++) {
				...
				if (factionAt != null && !(factionAt instanceof WildernessFaction)) {
					player.sendMessage(ChatColor.RED + "This claim is not in " + ChatColor.GRAY + "Wilderness"
							+ ChatColor.RED + '.');
					return false;
				}
			}
		...
		for (int x = minimumX - CLAIM_BUFFER_RADIUS; x < maximumX + CLAIM_BUFFER_RADIUS; x++) {
			for (int z = minimumZ - CLAIM_BUFFER_RADIUS; z < maximumZ + CLAIM_BUFFER_RADIUS; z++) {
				...
				if (!SettingsYML.ALLOW_CLAIMING_BESIDES_ROADS && factionAtNew instanceof ClaimableFaction
						&& playerFaction != factionAtNew && !(factionAtNew instanceof RoadFaction)) {
					player.sendMessage(ChatColor.RED + "The claim is " + CLAIM_BUFFER_RADIUS + " within your enemy.");
					return false;
				}
			}
		}
		...
		if (!conjoined) {
			...
			if (!conjoined) {
				player.sendMessage(ChatColor.RED + "All claims must be together.");
				return false;
			}
		}
		...
		player.sendMessage(ChatColor.AQUA + "You have purchased the claim for " + ChatColor.GREEN
				+ EconomyManager.ECONOMY_SYMBOL + claimPrice + ChatColor.AQUA + '.');
		playerFaction.setBalance(factionBalance - claimPrice);
		playerFaction.broadcast(ChatColor.GOLD + player.getName() + ChatColor.GREEN
				+ " claimed some land for your faction at " + ChatColor.GOLD + '(' + center.getBlockX() + ", "
