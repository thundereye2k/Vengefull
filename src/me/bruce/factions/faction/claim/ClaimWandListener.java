...
public class ClaimWandListener implements Listener {
	...
	public ClaimWandListener(LorexHCF plugin) {
		this.plugin = plugin;
	}
			...
			if (plugin.getClaimHandler().canClaimHere(player, blockLocation)) {
				...
				if (opposite != null) {
					...
					if (xDiff < ClaimHandler.MIN_CLAIM_RADIUS || zDiff < ClaimHandler.MIN_CLAIM_RADIUS) {
						player.sendMessage(ChatColor.RED + "Claim selections must be at least "
								+ ClaimHandler.MIN_CLAIM_RADIUS + 'x' + ClaimHandler.MIN_CLAIM_RADIUS + " blocks.");
						return;
					}
				}
				...
				player.sendMessage(ChatColor.GREEN + "You have selected Position " + ChatColor.YELLOW + selectionId
						+ ChatColor.GREEN + " at: " + ChatColor.GOLD + '(' + ChatColor.YELLOW + blockX + ", " + blockZ
						+ ChatColor.GOLD + ')');
				...
				if (claimSelection.hasBothPositionsSet()) {
					...
					player.sendMessage(ChatColor.AQUA + "The claim costs: "
							+ (selectionPrice > playerFaction.getBalance() ? ChatColor.RED : ChatColor.GREEN)
							+ EconomyManager.ECONOMY_SYMBOL + selectionPrice + ChatColor.AQUA + ". Size: ("
							+ ChatColor.WHITE + claim.getWidth() + ", " + claim.getLength() + ChatColor.AQUA + "), "
