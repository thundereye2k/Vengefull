...
import me.bruce.factions.listener.render.VisualBlock;
...
public class SubclaimWandListener implements Listener {
	...
	public SubclaimWandListener(LorexHCF plugin) {
		this.plugin = plugin;
	}
	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		 ...
		 * player.isSneaking()) { //TODO: Possibly open an Anvil to make a name for the
		 * Subclaim? }
		 */
		...
		if (action == Action.LEFT_CLICK_BLOCK || action == Action.RIGHT_CLICK_BLOCK) {
			...
			if (plugin.getClaimHandler().canSubclaimHere(player, blockLocation)) {
				...
				ClaimSelection claimSelection = plugin.getClaimHandler().claimSelectionMap.putIfAbsent(uuid,
						revert = new ClaimSelection(blockLocation.getWorld()));
				...
				// Allow at least 1 tick before players can update one of the positions to
				// prevent lag/visual glitches with delayed task below.
				if ((System.currentTimeMillis()
						- claimSelection.getLastUpdateMillis()) <= ClaimHandler.PILLAR_BUFFER_DELAY_MILLIS) {
					return;
				}

				if (opposite != null && (Math.abs(opposite.getBlockX() - blockX) <= ClaimHandler.MIN_SUBCLAIM_RADIUS
						|| Math.abs(opposite.getBlockZ() - blockZ) <= ClaimHandler.MIN_SUBCLAIM_RADIUS)) {
					player.sendMessage(ChatColor.RED + "Subclaim selections must be at least "
							+ ClaimHandler.MIN_SUBCLAIM_RADIUS + 'x' + ClaimHandler.MIN_SUBCLAIM_RADIUS + " blocks.");
					return;
				}

				if (oldPosition != null) {
					plugin.getVisualiseHandler().clearVisualBlocks(player, VisualType.CREATE_CLAIM_SELECTION,
							new Predicate<VisualBlock>() {
								@Override
								public boolean apply(VisualBlock visualBlock) {
									Location location = visualBlock.getLocation();
									return location.getBlockX() == oldPosition.getBlockX()
											&& location.getBlockZ() == oldPosition.getBlockZ();
								}
							});
				}
				...
				player.sendMessage(ChatColor.GREEN + "Set the location of subclaim selection " + ChatColor.YELLOW
						+ selectionId + ChatColor.GREEN + " to: " + ChatColor.GOLD + '(' + ChatColor.YELLOW + blockX
						+ ", " + blockZ + ChatColor.GOLD + ')');
				...
				new BukkitRunnable() {
					@Override
					public void run() {
						plugin.getVisualiseHandler().generate(player, locations, VisualType.CREATE_CLAIM_SELECTION,
								true);
					}
				}.runTask(plugin);
			}
