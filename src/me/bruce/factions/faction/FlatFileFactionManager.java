...
public class FlatFileFactionManager implements Listener, FactionManager {
	...
	public FlatFileFactionManager(LorexHCF plugin) {
	...
	public Faction getFactionAt(World world, int x, int z) {
		...
		// be efficient.
		int warzoneRadius = SettingsYML.WARZONE_RADIUS;
	...
	public void reloadFactionData() {
		...
		if (!factionNameMap.containsKey("GlowstoneMountain")) {
		...
		for (Faction added : adding) {
			...
			Bukkit.getConsoleSender()
					.sendMessage(ChatColor.BLUE + "FactionListener " + added.getName() + " not found, created.");
		}
	}
