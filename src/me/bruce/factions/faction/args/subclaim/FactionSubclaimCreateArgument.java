...
import me.bruce.factions.faction.claim.ClaimSelection;
...
public class FactionSubclaimCreateArgument extends CommandArgument {
	...
	public FactionSubclaimCreateArgument(LorexHCF plugin) {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (args[2].length() < SettingsYML.SUBCLAIM_NAME_CHARACTERS_MIN) {
			sender.sendMessage(ChatColor.RED + "Subclaim names must have at least "
					+ SettingsYML.SUBCLAIM_NAME_CHARACTERS_MIN + " characters.");
			return true;
		}

		if (args[2].length() > SettingsYML.SUBCLAIM_NAME_CHARACTERS_MAX) {
			sender.sendMessage(ChatColor.RED + "Subclaim names cannot be longer than "
					+ SettingsYML.SUBCLAIM_NAME_CHARACTERS_MAX + " characters.");
			return true;
		}
