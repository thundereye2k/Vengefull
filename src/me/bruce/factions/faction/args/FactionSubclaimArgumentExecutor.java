...
import me.bruce.factions.faction.args.subclaim.*;
...
public class FactionSubclaimArgumentExecutor extends CommandArgument {
	...
	public FactionSubclaimArgumentExecutor(LorexHCF plugin) {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		if (args.length < 2) {
			sender.sendMessage(ChatColor.AQUA + "*** FactionListener Subclaim Help ***");
			for (CommandArgument argument : arguments) {
				String permission = argument.getPermission();
				if (permission == null || sender.hasPermission(permission)) {
					sender.sendMessage(
							ChatColor.GRAY + argument.getUsage(label) + " - " + argument.getDescription() + '.');
				}
			}
		...
		if (argument == null || (permission != null && !sender.hasPermission(permission))) {
			sender.sendMessage(ChatColor.RED + "FactionListener subclaim sub-command " + args[1] + " not found.");
			return true;
		}
	...
	private static CommandArgument getArgument(Collection<CommandArgument> arguments, String id) {
		for (CommandArgument commandArgument : arguments) {
			if (commandArgument.getName().equalsIgnoreCase(id)
					|| Arrays.asList(commandArgument.getAliases()).contains(id)) {
