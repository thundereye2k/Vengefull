...
public class FactionRenameArgument extends CommandArgument {
	...
	public FactionRenameArgument(LorexHCF plugin) {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (newName.length() < SettingsYML.FACTION_NAME_CHARACTERS_MIN) {
			sender.sendMessage(ChatColor.RED + "FactionListener names must have at least "
					+ SettingsYML.FACTION_NAME_CHARACTERS_MIN + " characters.");
			return true;
		}

		if (newName.length() > SettingsYML.FACTION_NAME_CHARACTERS_MAX) {
			sender.sendMessage(ChatColor.RED + "FactionListener names cannot be longer than "
					+ SettingsYML.FACTION_NAME_CHARACTERS_MAX + " characters.");
			return true;
		}

		if (!JavaUtils.isAlphanumeric(newName)) {
			sender.sendMessage(ChatColor.RED + "FactionListener names may only be alphanumeric.");
			return true;
		}

		if (plugin.getFactionManager().getFaction(newName) != null) {
			sender.sendMessage(ChatColor.RED + "FactionListener " + newName + ChatColor.RED + " already exists.");
			return true;
		}
		...
		if (!player.isOp() && difference > 0L) {
			player.sendMessage(ChatColor.RED + "There is a faction rename delay of " + FACTION_RENAME_DELAY_WORDS
					+ ". Therefore you need to wait another "
