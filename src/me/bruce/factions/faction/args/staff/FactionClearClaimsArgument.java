...
/**
 * FactionListener argument used to set the DTR Regeneration cooldown of
 * {@link FactionListener}s.
 */
public class FactionClearClaimsArgument extends CommandArgument {
	...
	public FactionClearClaimsArgument(final LorexHCF plugin) {
		super("clearclaims", "Clears the claims of a faction.");
		this.plugin = plugin;
		this.permission = "hcf.command.faction.argument." + getName();

		this.factory = new ConversationFactory(plugin).withFirstPrompt(new ClaimClearAllPrompt(plugin))
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		sender.sendMessage(ChatColor.GOLD.toString() + ChatColor.BOLD + "Claims belonging to " + faction.getName()
				+ " have been forcefully wiped.");
		return true;
	}
	...
	private static class ClaimClearAllPrompt extends StringPrompt {
		...
		public ClaimClearAllPrompt(LorexHCF plugin) {
			this.plugin = plugin;
		}
