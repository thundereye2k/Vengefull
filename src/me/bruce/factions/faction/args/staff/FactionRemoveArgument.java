...
/**
 * FactionListener argument used to forcefully remove {@link FactionListener}s.
 */
public class FactionRemoveArgument extends CommandArgument {
	...
	public FactionRemoveArgument(final LorexHCF plugin) {
		super("remove", "Remove a faction.");
		this.plugin = plugin;
		this.aliases = new String[] { "delete", "forcedisband", "forceremove" };
		this.permission = "hcf.command.faction.argument." + getName();
		this.factory = new ConversationFactory(plugin).withFirstPrompt(new RemoveAllPrompt(plugin))
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (plugin.getFactionManager().removeFaction(faction, sender)) {
			Command.broadcastCommandMessage(sender,
					ChatColor.YELLOW + "Disbanded faction " + faction.getName() + ChatColor.YELLOW + '.');
		}
	...
	private static class RemoveAllPrompt extends StringPrompt {
		...
		public RemoveAllPrompt(LorexHCF plugin) {
			this.plugin = plugin;
		}
