...
public class FactionDepositArgument extends CommandArgument {
	...
	public FactionDepositArgument(LorexHCF plugin) {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (playerBalance < amount) {
			sender.sendMessage(ChatColor.RED + "You need at least " + EconomyManager.ECONOMY_SYMBOL
					+ JavaUtils.format(amount) + " to do this, you only have " + EconomyManager.ECONOMY_SYMBOL
		...
		playerFaction.broadcast(Relation.MEMBER.toChatColour() + playerFaction.getMember(player).getRole().getAstrix()
				+ sender.getName() + ChatColor.YELLOW + " has deposited " + ChatColor.GREEN
				+ EconomyManager.ECONOMY_SYMBOL + JavaUtils.format(amount) + ChatColor.YELLOW
				+ " into the faction balance.");
