...
public class EventDeleteArgument extends CommandArgument {
	...
	public EventDeleteArgument(LorexHCF plugin) {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (plugin.getFactionManager().removeFaction(faction, sender)) {
			sender.sendMessage(ChatColor.YELLOW + "Deleted event faction " + ChatColor.WHITE
	...
	public List<String> onTabComplete(CommandSender sender, Command command, String label, String[] args) {
		...
		return plugin.getFactionManager().getFactions().stream().filter(faction -> faction instanceof EventFaction)
