...
import LorexMC.us.utils.compat.com.google.common.collect.GuavaCompat;
...
public class ConquestTracker implements EventTracker, Listener {
	...
	private static final Comparator<Map.Entry<PlayerFaction, Integer>> POINTS_COMPARATOR = (e1, e2) -> e2.getValue()
	...
	public ConquestTracker(LorexHCF plugin) {
		this.plugin = plugin;
	...
	/**
	 * Gets the amount of points a {@link PlayerFaction} has gained for this
	 * {@link ConquestTracker}.
	 *
	 * @param faction
	 *            the faction to get for
	...
	/**
	 * Sets the points a {@link PlayerFaction} has gained for this
	 * {@link ConquestTracker}.
	 *
	 * @param faction
	 *            the faction to set for
	 * @param amount
	 *            the amount to set
	 * @return the new points of the {@link PlayerFaction}
	 */
	public int setPoints(PlayerFaction faction, int amount) {
		...
		synchronized (factionPointsMap) {
			...
			List<Map.Entry<PlayerFaction, Integer>> entries = Ordering.from(POINTS_COMPARATOR)
					.sortedCopy(factionPointsMap.entrySet());
	...
	/**
	 * Takes points from a {@link PlayerFaction} gained from this
	 * {@link ConquestTracker}.has
	 *
	 * @param faction
	 *            the faction to take from
	 * @param amount
	 *            the amount to take
	 * @return the new points of the {@link PlayerFaction}
	 */
	...
	/**
	 * Adds points to a {@link PlayerFaction} gained from this
	 * {@link ConquestTracker}.has
	 *
	 * @param faction
	 *            the faction to add from
	 * @param amount
	 *            the amount to add
	 * @return the new points of the {@link PlayerFaction}
	 */
			...
			if (remainingMillis <= 0L) {
				...
				if (playerFaction != null) {
					...
					if (newPoints < SettingsYML.CONQUEST_REQUIRED_WIN_POINTS) {
						...
						Bukkit.broadcastMessage(ChatColor.YELLOW + "[" + eventFaction.getName() + "] " + ChatColor.RED
								+ ChatColor.BOLD + playerFaction.getName() + ChatColor.GOLD + " gained " + 1
								+ " point for capturing " + captureZone.getDisplayName() + ChatColor.GOLD + ". "
	...
	public void onContest(EventFaction eventFaction, EventTimer eventTimer) {
		Bukkit.broadcastMessage(
				ChatColor.YELLOW + (eventFaction instanceof ConquestFaction ? "" : "[" + eventFaction.getName() + "] ")
						+ ChatColor.GOLD + eventFaction.getName() + " can now be contested.");
	}
	...
	public void onControlLoss(Player player, CaptureZone captureZone, EventFaction eventFaction) {
		...
		if (remainingMillis > 0L
			...
			Bukkit.broadcastMessage(ChatColor.YELLOW + "[" + eventFaction.getName() + "] " + ChatColor.RED
					+ ChatColor.BOLD + player.getName() + ChatColor.GOLD + " was knocked off "
	...
	public void onPlayerDeath(PlayerDeathEvent event) {
		...
		if (currentEventFac instanceof ConquestFaction) {
			...
			if (playerFaction != null && SettingsYML.CONQUEST_POINT_LOSS_PER_DEATH > 0) {
				...
				event.setDeathMessage(null); // for some reason if it isn't handled manually, weird colour coding
												// happens
				Bukkit.broadcastMessage(ChatColor.YELLOW + "[" + currentEventFac.getName() + "] " + ChatColor.GOLD
						+ playerFaction.getName() + " lost " + SettingsYML.CONQUEST_POINT_LOSS_PER_DEATH
						+ " points because " + player.getName() + " died." + ChatColor.AQUA + " (" + newPoints + '/'
						+ SettingsYML.CONQUEST_REQUIRED_WIN_POINTS + ')' + ChatColor.YELLOW + '.');
			}
		}
	}
}
