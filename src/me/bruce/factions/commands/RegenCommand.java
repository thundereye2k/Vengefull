...
public class RegenCommand implements CommandExecutor, TabCompleter {
	...
	public RegenCommand(LorexHCF plugin) {
		this.plugin = plugin;
	}
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		case REGENERATING:
			sender.sendMessage(
					ChatColor.YELLOW + "Your faction currently has " + ChatColor.YELLOW + regenStatus.getSymbol() + ' '
							+ playerFaction.getDeathsUntilRaidable() + ChatColor.YELLOW
							+ " DTR and is regenerating at a rate of " + ChatColor.GOLD
							+ SettingsYML.DTR_INCREMENT_BETWEEN_UPDATES + ChatColor.YELLOW + " every " + ChatColor.GOLD
							+ SettingsYML.DTR_WORDS_BETWEEN_UPDATES + ChatColor.YELLOW
							+ ". Your ETA for maximum DTR is " + ChatColor.RED + DurationFormatUtils
									.formatDurationWords(getRemainingRegenMillis(playerFaction), true, true)
							+ ChatColor.YELLOW + '.');
		...
		sender.sendMessage(
				ChatColor.RED + "Unrecognised regeneration status, please inform an Developer or an System Admin.");
		return true;
	}
	...
	public long getRemainingRegenMillis(PlayerFaction faction) {
		...
		return (long) ((SettingsYML.DTR_MILLIS_BETWEEN_UPDATES / SettingsYML.DTR_INCREMENT_BETWEEN_UPDATES)
