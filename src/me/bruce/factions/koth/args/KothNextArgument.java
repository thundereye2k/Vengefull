...
/**
 * An {@link CommandArgument} used to check the next running KingOfTheHill
 * event.
 */
public class KothNextArgument extends CommandArgument {
	...
	public KothNextArgument(LorexHCF plugin) {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		sender.sendMessage(ChatColor.GOLD + "The server time is currently " + ChatColor.YELLOW
		...
		for (Map.Entry<LocalDateTime, String> entry : scheduleMap.entrySet()) {
			...
			sender.sendMessage(ChatColor.DARK_AQUA + WordUtils.capitalizeFully(entry.getValue()) + ChatColor.GRAY
					+ " is the next event: " + ChatColor.AQUA + weekName + ' ' + scheduleDateTime.getDayOfMonth() + ' '
					+ monthName + ChatColor.DARK_AQUA + " ("
					+ DateTimeFormats.HR_MIN_AMPM.format(TimeUnit.HOURS.toMillis(scheduleDateTime.getHour())
