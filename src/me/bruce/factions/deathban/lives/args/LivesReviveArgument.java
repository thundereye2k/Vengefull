...
public class LivesReviveArgument extends CommandArgument {
	...
	public LivesReviveArgument(LorexHCF plugin) {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (sender instanceof Player) {
			...
			sender.sendMessage(ChatColor.YELLOW + "You have used a life to revive " + relation.toChatColour()
					+ target.getName() + ChatColor.YELLOW + '.');
		} else {
			sender.sendMessage(ChatColor.YELLOW + "You have revived " + SettingsYML.ENEMY_COLOUR + target.getName()
					+ ChatColor.YELLOW + '.');
		}
		...
		if (sender instanceof PluginMessageRecipient) {
			...
			String serverDisplayName = ChatColor.GREEN + "LorexHCF"; // TODO: Non hard-coded server display name.
