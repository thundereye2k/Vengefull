...
import me.bruce.factions.listener.DelayedMessageRunnable;
...
/*     */ {
	...
	/* 32 */ private static final String LIFE_USE_DELAY_WORDS = org.apache.commons.lang3.time.DurationFormatUtils
			.formatDurationWords(LIFE_USE_DELAY_MILLIS, true, true);
	/*     */
	...
	/*     */
	/*     */ public DeathbanListener(LorexHCF plugin) {
	...
	/*     */ public void onPlayerLogin(PlayerLoginEvent event) {
		...
		/* 56 */ if (this.plugin.getEotwHandler().isEndOfTheWorld()) {
			/* 57 */ event.disallow(PlayerLoginEvent.Result.KICK_OTHER, ChatColor.RED
					+ "Deathbanned for the entirety of the map due to EOTW.\nVisit www.zorex.us to view SOTW info.");
			/* 58 */ return;
			/*     */ }
		/*     */
		...
		/*     */
		/* 66 */ if (lives <= 0) {
			/* 67 */ event.disallow(PlayerLoginEvent.Result.KICK_OTHER,
					ChatColor.RED + "You are still Deathbanned \n Duration: " + ChatColor.WHITE + formattedRemaining);
			/*     */
			/* 70 */ return;
			/*     */ }
		/*     */
		...
		/*     */
		/*     */
		/* 77 */ if ((lastAttemptedJoinMillis != this.lastAttemptedJoinMap.getNoEntryValue())
			...
			/* 83 */ new DelayedMessageRunnable(player,
					ChatColor.YELLOW + "You have used a life for entry. You now have " + ChatColor.WHITE + lives
							+ ChatColor.YELLOW + " lives.").runTask(this.plugin);
			/*     */
			/* 85 */ return;
			/*     */ }
		/*     */
		...
		/*     */
		/* 92 */ event.disallow(PlayerLoginEvent.Result.KICK_OTHER, ChatColor.RED
				+ "You are currently Deathbanned for Duration: " + ChatColor.WHITE + formattedRemaining + " \n" +
				/* 93 */ ChatColor.RED + "You currently have " + ChatColor.WHITE
				+ (lives <= 0 ? "no" : Integer.valueOf(lives)) + " lives." + ChatColor.RED
	...
	/*     */ public void onPlayerDeath(PlayerDeathEvent event) {
		...
		/* 100 */ final Deathban deathban = this.plugin.getDeathbanManager().applyDeathBan(player,
	...
	/*     */ public void onPlayerRequestRespawn(PlayerRespawnEvent event) {
		...
		/* 125 */ if ((deathban != null) && (deathban.getRemaining() > 0L)) {
			/* 126 */ if (player.hasPermission("hcf.deathban.bypass")) {
				...
				/* 129 */ new DelayedMessageRunnable(player, ChatColor.RED + "Bypass access granted.")
						.runTask(this.plugin);
				/*     */
				/* 131 */ return;
				/*     */ }
	...
	/*     */ private void handleKick(Player player, Deathban deathban) {
		/* 155 */ if (this.plugin.getEotwHandler().isEndOfTheWorld()) {
			/* 156 */ player.kickPlayer(ChatColor.RED
					+ "Deathbanned for the entirety of the map due to EOTW.\nVisit www.zorex.us for SOTW info!");
			/*     */ } else {
			/* 158 */ player.kickPlayer(ChatColor.RED + "You are now Deathbanned \n Duration: " + ChatColor.WHITE
