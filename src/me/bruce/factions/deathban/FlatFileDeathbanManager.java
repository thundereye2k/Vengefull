...
public class FlatFileDeathbanManager implements DeathbanManager {
	...
	public FlatFileDeathbanManager(LorexHCF plugin) {
		this.plugin = plugin;
	...
	public Deathban applyDeathBan(Player player, String reason) {
		...
		} else if (player.hasPermission("hcf.deathban.rank.bronze")) {
			duration = JavaUtils.parse("90m");
		} else if (player.hasPermission("hcf.deathban.rank.silver")) {
			duration = JavaUtils.parse("75m");
		} else if (player.hasPermission("hcf.deathban.rank.cobalt")) {
			duration = JavaUtils.parse("60m");
		} else if (player.hasPermission("hcf.deathban.rank.ruby")) {
			duration = JavaUtils.parse("40m");
		} else if (player.hasPermission("hcf.deathban.rank.sapphire")) {
			duration = JavaUtils.parse("30m");
		} else if (player.hasPermission("hcf.deathban.rank.titanium")) {
			duration = JavaUtils.parse("20m");
		} else if (player.hasPermission("hcf.deathban.rank.amethyst")) {
			duration = JavaUtils.parse("10m");
		} else if (player.hasPermission("hcf.deathban.rank.riots")) {
			duration = JavaUtils.parse("5m");
		} else if (player.hasPermission("hcf.deathban.rank.default")) {
			duration = JavaUtils.parse("120m");
		}
