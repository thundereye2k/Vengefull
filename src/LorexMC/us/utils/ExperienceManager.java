...
public class ExperienceManager {
	...
	private static void initLookupTables(final int maxLevel) {
		...
		for (int i = 0; i < ExperienceManager.xpTotalToReachLevel.length; ++i) {
			ExperienceManager.xpTotalToReachLevel[i] = ((i >= 30) ? ((int) (3.5 * i * i - 151.5 * i + 2220.0))
					: ((i >= 16) ? ((int) (1.5 * i * i - 29.5 * i + 360.0)) : (17 * i)));
		}
	}
	...
	public int getLevelForExp(final int exp) {
		...
		if (exp > ExperienceManager.xpTotalToReachLevel[ExperienceManager.xpTotalToReachLevel.length - 1]) {
			...
			Preconditions.checkArgument(newMax <= ExperienceManager.hardMaxLevel,
					"Level for exp " + exp + " > hard max level " + ExperienceManager.hardMaxLevel);
	...
	public int getXpForLevel(final int level) {
		Preconditions.checkArgument(level >= 0 && level <= ExperienceManager.hardMaxLevel,
				"Invalid level " + level + "(must be in range 0.." + ExperienceManager.hardMaxLevel + ')');
