...
abstract public class THash implements Externalizable {
	 ...
	 * prime value because one is added to it when searching for a prime capacity to
	 * account for the free slot required by open addressing. Thus, the real default
	 * capacity is 11.
	 */
	...
	/**
	 * Creates a new <code>THash</code> instance with the default capacity and load
	 * factor.
	 */
	public THash() {
	...
	/**
	 * Creates a new <code>THash</code> instance with a prime capacity at or near
	 * the minimum needed to hold <tt>initialCapacity</tt> elements with load factor
	...
	public void ensureCapacity(int desiredCapacity) {
		if (desiredCapacity > (_maxSize - size())) {
			rehash(PrimeFinder.nextPrime(
					Math.max(size() + 1, HashFunctions.fastCeil((desiredCapacity + size()) / _loadFactor) + 1)));
	 ...
	 * queries or insertions or iteration, it is a good idea to invoke this method.
	 * Doing so will accomplish two things:
	 * <p/>
	 * <ol>
	 * <li>You'll free memory allocated to the table but no longer needed because of
	 * the remove()s.</li>
	 * <p/>
	...
	/**
	 * This simply calls {@link #compact compact}. It is included for symmetry with
	 * other collection classes. Note that the name of this method is somewhat
	 * misleading (which is why we prefer <tt>compact</tt>) as the load factor may
