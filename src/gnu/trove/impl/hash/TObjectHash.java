...
abstract public class TObjectHash<T> extends THash {
	...
	/**
	 * Creates a new <code>TObjectHash</code> instance with the default capacity and
	 * load factor.
	 */
	public TObjectHash() {
		super();
	 ...
	 * highest prime above <tt>initialCapacity + 1</tt> unless that value is already
	 * prime.
	 *
	...
	/**
	 * Creates a new <code>TObjectHash</code> instance with a prime value at or near
	 ...
	 * <p/>
	 * null specific loop exploiting several properties to simplify the iteration
	 * logic - the null value hashes to 0 we so we can iterate from the beginning. -
	 * the probe value is 1 for this case - object identity can be used to match
	 * this case
	 * <p/>
	...
	/**
	 * Alias introduced to avoid breaking the API. The new method name insertKey()
	 * reflects the changes made to the logic.
	 *
	 * @param obj
	 * @return
	...
	/**
	 * Locates the index at which <tt>key</tt> can be inserted. if there is already
	 * a value equal()ing <tt>key</tt> in the set, returns that value's index as
	 * <tt>-index - 1</tt>.
	 * <p/>
	 * If a slot is found the value is inserted. When a FREE slot is used the
	 * consumeFreeSlot field is set to true. This field should be used in the method
	 * invoking insertKey() to pass to postInsertHook()
	 *
	 * @param key
	 *            an <code>Object</code> value
	 * @return the index of a FREE slot at which key can be inserted or, if key is
	...
	/**
	 * Looks for a slot using double hashing for a null key value and inserts the
	 * value.
	 * <p/>
	 * null specific loop exploiting several properties to simplify the iteration
	 * logic - the null value hashes to 0 we so we can iterate from the beginning. -
	...
	/**
	 * Convenience methods for subclasses to use in throwing exceptions about badly
	 * behaved user objects employed as keys. We have to throw an
	 * IllegalArgumentException with a rather verbose message telling the user that
	 * they need to fix their object implementation to conform to the general
	...
	protected final IllegalArgumentException buildObjectContractViolation(Object o1, Object o2, String extra) {
		return new IllegalArgumentException("Equal objects must have equal hashcodes. "
				...
				+ "bullet point two in that method's documentation. object #1 =" + objectInfo(o1) + "; object #2 ="
				+ objectInfo(o2) + "\n" + extra);
	}
	 ...
	 * @param currentSize
	 *            size of the key set during rehashing
	 * @param oldSize
	 *            size of the key set before rehashing
	 * @param oldKeys
	 *            the old key set
	 */
	...
	protected static String objectInfo(Object o) {
		return (o == null ? "class null" : o.getClass()) + " id= " + System.identityHashCode(o) + " hashCode= "
				+ (o == null ? 0 : o.hashCode()) + " toString= " + String.valueOf(o);
	}
