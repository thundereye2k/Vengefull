 ...
 * the ListCommand implement the <tt>TLinkable</tt> interface.
 * <p/>
 * The limitations are:
 * <ul>
 * <li>the same object cannot be put into more than one list at the same time.
 * <li>the same object cannot be put into the same list more than once at the
 * same time.
 * <li>objects must only be removed from list they are in. That is, if you have
 * an object A and lists l1 and l2, you must ensure that you invoke
 * ListCommand.remove(A) on the correct list.
 * <li>It is also forbidden to invoke ListCommand.remove() with an unaffiliated
 * TLinkable (one that belongs to no list): this will destroy the list you
 * invoke it on.
 * </ul>
 *
...
public class TLinkedList<T extends TLinkable<T>> extends AbstractSequentialList<T> implements Externalizable {
	...
	/**
	 * Returns an iterator positioned at <tt>index</tt>. Assuming that the list has
	 * a value at that index, calling next() will retrieve and advance the iterator.
	 * Assuming that there is a value before <tt>index</tt> in the list, calling
	 * previous() will retrieve it (the value at index - 1) and move the iterator to
	 * that position. So, iterating from front to back starts at 0; iterating from
	 * back to front starts at <tt>size()</tt>.
	 *
	 * @param index
	 ...
	 * index are shifted over one position to accommodate the new addition.
	 *
	 * @param index
	 *            an <code>int</code> value
	 * @param linkable
	...
	/**
	 * Copies the list's contents into a native array. This will be a shallow copy:
	 * the Tlinkable instances in the Object[] array have links to one another:
	 * changing those will put this list into an unpredictable state. Holding a
	 * reference to one element in the list will prevent the others from being
	 * garbage collected unless you clear the next/previous links. <b>Caveat
	 * programmer!</b>
	 *
	...
	/**
	 * Copies the list to a native array, destroying the next/previous links as the
	 * copy is made. This list will be emptied after the copy (as if clear() had
	 * been invoked). The Object[] array returned will contain TLinkables that do
	 * <b>not</b> hold references to one another and so are less likely to be the
	 ...
	 * </ol>
	 * <p/>
	 * NOTE: this should only be used with nodes contained in the list. The results
	 * are undefined with anything else.
	 *
	 * @param current
	 *            The current node
	 ...
	 * responsibility to ensure that the element does, in fact, belong to this list
	 * and not another instance of TLinkedList.
	 *
	 * @param o
	...
	/**
	 * Inserts newElement into the list immediately before current. All elements to
	 * the right of and including current are shifted over.
	 *
	 * @param current
	...
	/**
	 * Inserts newElement into the list immediately after current. All elements to
	 * the left of and including current are shifted over.
	 *
	 * @param current
	 *            a <code>TLinkable</code> value currently in the list.
	 * @param newElement
	...
	protected final class IteratorImpl implements ListIterator<T> {
		...
		/**
		 * Returns the value before the Iterator's index and moves the iterator back one
		 * index.
		 *
		 * @return an <code>Object</code> value
		 * @throws NoSuchElementException
		 ...
		 *             have been invoked after the last invocation of next or previous.
		 */
		@Override
		@SuppressWarnings({ "unchecked" })
		...
		private void swap(T from, T to) {
			...
			if (_lastReturned == from)
				_lastReturned = to;
			else if (_lastReturned == to)
				_lastReturned = from;
