...
/**
 * Predicate that compares the input object with the one stored in the predicate
 * using a comparator. In addition, the comparator result can be evaluated in
 * accordance to a supplied criterion value.
 *
 * In order to demonstrate the use of the predicate, the following variables are
 * declared:
 *
 * <pre>
 ...
 * </pre>
 *
 * Using the declared variables, the <code>ComparatorPredicate</code> can be
 * used used in the following way:
 *
 * <pre>
 * ComparatorPredicate.comparatorPredicate(ONE, comparator).evaluate(TWO);
 * </pre>
 *
 * The input variable <code>TWO</code> in compared to the stored variable
 * <code>ONE</code> using the supplied <code>comparator</code>. This is the
 * default usage of the predicate and will return <code>true</code> if the
 * underlying comparator returns <code>0</code>. In addition to the default
 * usage of the predicate, it is possible to evaluate the comparator's result in
 * several ways. The following {@link Criterion} enumeration values are provided
 * by the predicate:
 * </p>
 *
 * <ul>
 ...
 * </ul>
 *
 * The following examples demonstrates how these constants can be used in order
 * to manipulate the evaluation of a comparator result.
 *
 * <pre>
 * ComparatorPredicate.comparatorPredicate(ONE, comparator,<b>ComparatorPredicate.Criterion.GREATER</b>).evaluate(TWO);
 * </pre>
 *
 * The input variable TWO is compared to the stored variable ONE using the
 * supplied <code>comparator</code> using the <code>GREATER</code> evaluation
 * criterion constant. This instructs the predicate to return <code>true</code>
 * if the comparator returns a value greater than <code>0</code>.
 *
 * @since 4.0
...
public class ComparatorPredicate<T> implements Predicate<T>, Serializable {
	...
	/**
	 * Constructor that performs no validation. Use <code>comparatorPredicate</code>
	 * if you want that.
	 *
	 * @param object
	 *            the object to compare to
	...
	/**
	 * Evaluates the predicate. The predicate evaluates to <code>true</code> in the
	 * following cases:
	 *
	 * <ul>
	 ...
	 * @see java.util.Comparator#compare(java.lang.Object first, java.lang.Object
	 *      second)
	 *
	 * @param target
	 *            the target object to compare to
	 * @return {@code true} if the comparison succeeds according to the selected
	 *         criterion
