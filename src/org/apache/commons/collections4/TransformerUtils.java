 ...
 * <li>StringValue - returns a <code>java.lang.String</code> representation of
 * the input object
 * </ul>
 * <p>
...
public class TransformerUtils {
	...
	/**
	 * Gets a transformer that always throws an exception. This could be useful
	 ...
	 * immutable to maintain the contract of Transformer (although this is not
	 * checked).
	 *
	 * @param <T>
	 ...
	 * will be cloned using one of these techniques (in order):
	 * <ul>
	...
	/**
	 * Creates a Transformer that calls a Predicate each time the transformer is
	 * used. The transformer will return either Boolean.TRUE or Boolean.FALSE.
	 *
	 * @param <T>
	 *            the input type
	 * @param predicate
	 ...
	 * @return the transformer
	 * @throws NullPointerException
	 *             if the transformers array or any of the transformers is null
	 ...
	 * @return the transformer
	 * @throws NullPointerException
	 *             if the transformers collection or any of the transformers is null
	 * @see ChainedTransformer
	 */
	...
	/**
	 * Create a new Transformer that calls one of two transformers depending on the
	 * specified predicate.
	 *
	 * @param <I>
	 *            the input type
	 ...
	 * @deprecated as of 4.1, use {@link #ifTransformer(Predicate, Transformer,
	 *             Transformer))
	 */
	...
	public static <I, O> Transformer<I, O> switchTransformer(final Predicate<? super I> predicate,
			final Transformer<? super I, ? extends O> trueTransformer,
			final Transformer<? super I, ? extends O> falseTransformer) {
		return SwitchTransformer.switchTransformer(new Predicate[] { predicate }, new Transformer[] { trueTransformer },
				falseTransformer);
	}

	/**
	 * Create a new Transformer that calls one of the transformers depending on the
	 * predicates. The transformer at array location 0 is called if the predicate at
	 * array location 0 returned true. Each predicate is evaluated until one returns
	 * true. If no predicates evaluate to true, null is returned.
	 *
	 * @param <I>
	 *            the input type
	 * @param <O>
	 ...
	 * predicates. The transformer at array location 0 is called if the predicate at
	 * array location 0 returned true. Each predicate is evaluated until one returns
	 * true. If no predicates evaluate to true, the default transformer is called.
	 ...
	 *            the default to call if no predicate matches, null means return
	 *            null
	 * @return the transformer
	 * @throws NullPointerException
	 *             if the either array is null
	 ...
	 * predicates.
	 * <p>
	 * The Map consists of Predicate keys and Transformer values. A transformer is
	 * called if its matching predicate returns true. Each predicate is evaluated
	 * until one returns true. If no predicates evaluate to true, the default
	 * transformer is called. The default transformer is set in the map with a null
	 * key. If no default transformer is set, null will be returned in a default
	 ...
	 * @param <O>
	 *            the output type
	 * @param predicatesAndTransformers
	 ...
	 * transformer to call.
	 * <p>
	 * The Map consists of object keys and Transformer values. A transformer is
	 * called if the input object equals the key. If there is no match, the default
	 * transformer is called. The default transformer is set in the map using a null
	 * key. If no default is set, null will be returned in a default case.
	 *
	 * @param <I>
	 *            the input type
	 * @param <O>
	...
	/**
	 * Gets a Transformer that expects an input Class object that it will
	 * instantiate.
	 *
	 * @param <T>
	 *            the output type
	 ...
	 * instantiate. The constructor used is determined by the arguments specified to
	 * this method.
	 *
	 * @param <T>
	 *            the output type
	 * @param paramTypes
	 *            parameter types for the constructor, can be null
	 * @param args
	 ...
	 * @return the transformer, or {@link ConstantTransformer#nullTransformer()} if
	 *         the {@code map} is {@code null}
	 * @see MapTransformer
	 */
	...
	/**
	 * Gets a Transformer that invokes a method on the input object. The method must
	 * have no parameters. If the input object is null, null is returned.
	 * <p>
	 * For example, <code>TransformerUtils.invokerTransformer("getName");</code>
	 * will call the <code>getName/code> method on the input object to determine the
	 * transformer result.
	 *
	 * @param <I>
	 *            the input type
	 ...
	 * parameters are specified. If the input object is {@code null}, {@code null}
	 * is returned.
	 *
	 * @param <I>
	 *            the input type
