 ...
 * <p>
 * Any action that would change the size of the map is disallowed. The put
 * method is allowed to change the value associated with an existing key
 * however.
 * <p>
 * If trying to remove or clear the map, an UnsupportedOperationException is
 * thrown. If trying to put a new mapping into the map, an
 * IllegalArgumentException is thrown. This is because the put method can
 * succeed if the mapping's key already exists in the map, so the put method is
 * not always unsupported.
 * <p>
 * <strong>Note that FixedSizeSortedMap is not synchronized and is not
