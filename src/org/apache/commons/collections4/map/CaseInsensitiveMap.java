 ...
 * contracts.</note> As a general rule, don't compare this map to other maps. In
 * particular, you can't use decorators like {@link ListOrderedMap} on it, which
 * silently assume that these contracts are fulfilled.
 * <p>
...
public class CaseInsensitiveMap<K, V> extends AbstractHashedMap<K, V> implements Serializable, Cloneable {
	 ...
	 * <p>
	 * Keys will be converted to lower case strings, which may cause some entries to
	 * be removed (if string representation of keys differ only by character case).
	 *
	 * @param map
	 *            the map to copy
