 ...
 * operations, assuming (approximate) uniform hashing and that the number of
 * entries does not exceed the number of buckets. If the number of entries
 * exceeds the number of buckets or if the hash codes of the objects are not
 * uniformly distributed, these operations have a worst case scenario that is
 * proportional to the number of elements in the map (<i>O(n)</i>).
 * <p>
 *
 ...
 * contention. This means that you don't have to wrap instances of this class
 * with {@link java.util.Collections#synchronizedMap(Map)}; instances are
 * already thread-safe. Unfortunately, however, this means that this map
 * implementation behaves in ways you may find disconcerting. Bulk operations,
 * such as {@link #putAll(Map) putAll} or the
 * {@link Collection#retainAll(Collection) retainAll} operation in collection
 * views, are <i>not</i> atomic. If two threads are simultaneously executing
 *
 * <pre>
 ...
 * other out, leaving <code>staticBucketMapInstance</code> essentially
 * unchanged, or they could leave some random subset of <code>map</code> in
 * <code>staticBucketMapInstance</code>.
 * <p>
 *
 ...
 * {@link java.util.ConcurrentModificationException}. Keys and values added to
 * the map after the iterator is created do not necessarily appear during
 * iteration. Similarly, the iterator does not necessarily fail to return keys
 * and values that were removed after the iterator was created.
 * <p>
 *
 * Finally, unlike {@link java.util.HashMap}-style implementations, this class
 * <i>never</i> rehashes the map. The number of buckets is fixed at construction
 * time and never altered. Performance may degrade if you do not allocate enough
 * buckets upfront.
 * <p>
 *
 ...
 *
 * Use this class if you do not require reliable bulk operations and iterations,
 * or if you can make your own guarantees about how bulk operations will affect
 * the map.
 * <p>
 *
...
public final class StaticBucketMap<K, V> extends AbstractIterableMap<K, V> {
	...
	/**
	 * Initializes the map with a specified number of buckets. The number of buckets
	 * is never below 17, and is always an odd number (StaticBucketMap ensures
	 * this). The number of buckets is inversely proportional to the chances for
	 * thread contention. The fewer buckets, the more chances for thread contention.
	 * The more buckets the fewer chances for thread contention.
	 *
	 * @param numBuckets
	...
	// -----------------------------------------------------------------------
	/**
	 * Determine the exact hash entry for the key. The hash algorithm is rather
	 ...
	 *
	 * <p>
	 * He is the entry's hashCode, Hk is the key's hashCode, and n is the number of
	 * buckets.
	 * </p>
	 */
	...
	/**
	 * Gets the current size of the map. The value is computed fresh each time the
	 * method is called.
	 *
	...
	// -----------------------------------------------------------------------
	/**
	 * Puts all the entries from the specified map into this map. This operation is
	 * <b>not atomic</b> and may have undesired effects.
	 *
	 * @param map
	 ...
	 * {@link Runnable} executes. This method can be used, for instance, to execute
	 * a bulk operation atomically:
	 *
	 * <pre>
	 ...
	 * </pre>
	 *
	 * <b>Implementation note:</b> This method requires a lot of time and a ton of
	 * stack space. Essentially a recursive algorithm is used to enter each bucket's
	 * monitor. If you have twenty thousand buckets in your map, then the recursive
	 * method will be invoked twenty thousand times. You have been warned.
	 *
	 * @param r
