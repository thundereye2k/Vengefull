...
/**
 * An abstract implementation of a hash-based map that links entries to create
 * an ordered map and which provides numerous points for subclasses to override.
 * <p>
 * This class implements all the features necessary for a subclass linked
 * hash-based map. Key-value entries are stored in instances of the
 * <code>LinkEntry</code> class which can be overridden and replaced. The
 ...
 * <p>
 * This implementation maintains order by original insertion, but subclasses may
 * work differently. The <code>OrderedMap</code> interface is implemented to
 * provide access to bidirectional iteration and extra convenience methods.
 * <p>
...
public abstract class AbstractLinkedMap<K, V> extends AbstractHashedMap<K, V> implements OrderedMap<K, V> {
	 ...
	 * {@link #createEntry(HashEntry, int, Object, Object)} to create the map entry
	 * object.
	 */
	@Override
	protected void init() {
	 ...
	 * <p>
	 * This implementation adds the entry to the data storage table and to the end
	 * of the linked list.
	 *
	 * @param entry
	 *            the entry to add
	 ...
	 * <p>
	 * This implementation removes the entry from the linked list chain, then calls
	...
	// -----------------------------------------------------------------------
	/**
	 * Gets the <code>before</code> field from a <code>LinkEntry</code>. Used in
	...
	/**
	 * Gets the <code>after</code> field from a <code>LinkEntry</code>. Used in
	 * subclasses that have no visibility of the field.
	 *
	 * @param entry
	 *            the entry to query, must not be null
	 ...
	 * <code>entryXxx()</code> methods on <code>AbstractLinkedMap</code> exist to
	...
	protected static class LinkEntry<K, V> extends HashEntry<K, V> {
		 ...
		 *
		 * @param next
		 *            the next entry in the hash bucket sequence
		 * @param hashCode
		 *            the hash code
		 * @param key
		 *            the key
