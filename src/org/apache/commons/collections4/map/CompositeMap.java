 ...
 * <p>
 * <strong>Note that CompositeMap is not synchronized and is not
...
public class CompositeMap<K, V> extends AbstractIterableMap<K, V> implements Serializable {
	 ...
	 *
	 * @param one
	 *            the first Map to be composited
	 * @param two
	 ...
	 * @throws IllegalArgumentException
	 *             if there is a key collision and there is no MapMutator set to
	 *             handle it.
	 */
	@SuppressWarnings("unchecked")
	 ...
	 * More formally, returns {@code true} if and only if this map contains at a
	 ...
	 *             if the key is {@code null} and this map does not not permit
	 ...
	 * @throws NullPointerException
	 *             if the value is {@code null} and this map does not not permit
	 ...
	 * returned set is a <code>Map.Entry</code>. The set is backed by the map, so
	 ...
	 * <p>
	 * This implementation returns a <code>CompositeSet</code> which composites the
	 * entry sets from all of the composited maps.
	 *
	 * @see CompositeSet
	 ...
	 * {@code v} such that <tt>(key==null ? k==null :
	 * key.equals(k))</tt>, then this method returns {@code v}; otherwise it returns
	 ...
	 *             if the key is of an inappropriate type for this map (optional).
	 * @throws NullPointerException
	 *             key is {@code null} and this map does not not permit {@code null}
	 *             keys (optional).
	 *
	 ...
	 * {@code retainAll}, and {@code clear} operations. It does not support the add
	 * or {@code addAll} operations.
	 * <p>
	 * This implementation returns a <code>CompositeSet</code> which composites the
	 * key sets from all of the composited maps.
	 *
	 ...
	 * operation is unspecified if the specified map is modified while the operation
	 * is in progress.
	 *
	 * @param map
	 ...
	 *
	 * @throws UnsupportedOperationException
	 *             if the {@code putAll} method is not supported by this map.
	 *
	 * @throws ClassCastException
	 ...
	 *
	 * @throws IllegalArgumentException
	 *             some aspect of a key or value in the specified map prevents it
	 *             from being stored in this map.
	 * @throws NullPointerException
	 *             the specified map is {@code null}, or if this map does not permit
	 *             {@code null} keys or values, and the specified map contains
	 ...
	 * to value {@code v} such that
	 * <code>(key==null ?  k==null : key.equals(k))</code>, that mapping is removed.
	 * (The map can contain at most one such mapping.)
	 *
	 * <p>
	 * Returns the value to which the map previously associated the key, or
	 * {@code null} if the map contained no mapping for this key. (A {@code null}
	 * return can also indicate that the map previously associated {@code null} with
	 * the specified key if the implementation supports {@code null} values.) The
	 * map will not contain a mapping for the specified key once the call returns.
	 *
	 * @param key
	 ...
	 *
	 * @throws ClassCastException
	 *             if the key is of an inappropriate type for the composited map
	 *             (optional).
	 * @throws NullPointerException
	 *             if the key is {@code null} and the composited map does not not
	 *             permit {@code null} keys (optional).
	 * @throws UnsupportedOperationException
	 *             if the {@code remove} method is not supported by the composited
	...
	/**
	 * This interface allows definition for all of the indeterminate mutators in a
	 * CompositeMap, as well as providing a hook for callbacks on key collisions.
	 */
	public static interface MapMutator<K, V> extends Serializable {
		 ...
		 * @param existing
		 *            the Map already in the composite which contains the offending key
		 * @param added
		 *            the Map being added
		 * @param intersect
