...
/**
 * A <code>Map</code> implementation that stores data in simple fields until the
 * size is greater than 3.
 * <p>
 * This map is designed for performance and can outstrip HashMap. It also has
 * good garbage collection characteristics.
 * <ul>
 ...
 * </ul>
 * The design uses two distinct modes of operation - flat and delegate. While
 * the map is size 3 or less, operations map straight onto fields using switch
 * statements. Once size 4 is reached, the map switches to delegate mode and
 * only switches back when cleared. In delegate mode, all operations are
 ...
 * collection.
 * <p>
 * Whilst in flat mode this map is also easy for the garbage collector to
 * dispatch. This is because it contains no complex objects or arrays which slow
 * the progress.
 * <p>
...
public class Flat3Map<K, V> implements IterableMap<K, V>, Serializable, Cloneable {
	 ...
	 * implementation. Not every AbstractHashedMap is suitable, identity and
	 * reference based maps would be poor choices.
	 *
	 ...
	 * create an entrySet/keySet/values object. It also avoids creating the Map
	 * Entry object.
	 *
	 ...
	 * object creation and simply iterate through the entries, use
	 * {@link #mapIterator()}.
	 *
