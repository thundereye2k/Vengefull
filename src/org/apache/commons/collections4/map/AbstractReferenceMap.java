...
public abstract class AbstractReferenceMap<K, V> extends AbstractHashedMap<K, V> {
	...
	/**
	 * Constructs a new empty map with the specified reference types, load factor
	 * and initial capacity.
	 *
	 * @param keyType
	 *            the type of reference to use for keys; must be
	 *            {@link ReferenceStrength#HARD HARD}, {@link ReferenceStrength#SOFT
	 *            SOFT}, {@link ReferenceStrength#WEAK WEAK}
	 * @param valueType
	 *            the type of reference to use for values; must be
	 *            {@link ReferenceStrength#HARD}, {@link ReferenceStrength#SOFT
	 *            SOFT}, {@link ReferenceStrength#WEAK WEAK}
	 * @param capacity
	 ...
	 * @param purgeValues
	 *            should the value be automatically purged when the key is garbage
	 *            collected
	 */
	protected AbstractReferenceMap(final ReferenceStrength keyType, final ReferenceStrength valueType,
	...
	/**
	 * Puts a key-value mapping into this map. Neither the key nor the value may be
	 * null.
	 *
	 * @param key
	...
	// -----------------------------------------------------------------------
	/**
	 * Gets a MapIterator over the reference map. The iterator only returns valid
	 * key/value pairs.
	 *
	...
	/**
	 * Returns a set view of this map's entries. An iterator returned entry is valid
	 * until <code>next()</code> is called again. The <code>setValue()</code> method
	 * on the <code>toArray</code> entries has no effect.
	 *
	...
	/**
	 * Gets the hash code for a MapEntry. Subclasses can override this, for example
	 ...
	 */
	@Override
	protected ReferenceEntry<K, V> createEntry(final HashEntry<K, V> next, final int hashCode, final K key,
			final V value) {
	...
	protected static class ReferenceEntry<K, V> extends HashEntry<K, V> {
		 ...
		 *            the value
		 */
		public ReferenceEntry(final AbstractReferenceMap<K, V> parent, final HashEntry<K, V> next, final int hashCode,
		...
		/**
		 * Gets the key from the entry. This method dereferences weak and soft keys and
		 * thus may return null.
		 *
		...
		/**
		 * Gets the value from the entry. This method dereferences weak and soft value
		 * and thus may return null.
		 *
		...
		/**
		 * Constructs a reference of the given type to the given referent. The reference
		 * is registered with the queue for later purging.
		 *
		 * @param <T>
		 ...
		 * @param hash
		 *            the hash code of the <i>key</i> of the mapping; this number might
		 *            be different from referent.hashCode() if the referent represents a
		 *            value and not a key
	...
	protected void doWriteObject(final ObjectOutputStream out) throws IOException {
		...
		// do not call super.doWriteObject() as code there doesn't work for reference
		// map
	}
