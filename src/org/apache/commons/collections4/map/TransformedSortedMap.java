...
/**
 * Decorates another <code>SortedMap </code> to transform objects that are
 * added.
 * <p>
 ...
 * <p>
 * <strong>Note that TransformedSortedMap is not synchronized and is not
 * thread-safe.</strong> If you wish to use this map from multiple threads
 * concurrently, you must use appropriate synchronization. The simplest approach
 * is to wrap this map using
 * {@link java.util.Collections#synchronizedSortedMap}. This class may throw
...
public class TransformedSortedMap<K, V> extends TransformedMap<K, V> implements SortedMap<K, V> {
	 ...
	 * transformed. Contrast this with
	 ...
	 * <p>
	 * If there are any elements already in the map being decorated, they will be
	 * transformed by this method. Contrast this with
	 * {@link #transformingSortedMap(SortedMap, Transformer, Transformer)}.
	 *
	 * @param <K>
	 *            the key type
	 * @param <V>
	 ...
	 *            the map to decorate, must not be null
	 * @param keyTransformer
	 *            the transformer to use for key conversion, null means no
	 *            transformation
	 * @param valueTransformer
	 *            the transformer to use for value conversion, null means no
	 *            transformation
	 * @return a new transformed sorted map
	 * @throws NullPointerException
	 *             if map is null
	...
	public static <K, V> TransformedSortedMap<K, V> transformedSortedMap(final SortedMap<K, V> map,
		...
		final TransformedSortedMap<K, V> decorated = new TransformedSortedMap<K, V>(map, keyTransformer,
				valueTransformer);
		if (map.size() > 0) {
	 ...
	 *             if the map is null
	 */
	protected TransformedSortedMap(final SortedMap<K, V> map, final Transformer<? super K, ? extends K> keyTransformer,
			final Transformer<? super V, ? extends V> valueTransformer) {
