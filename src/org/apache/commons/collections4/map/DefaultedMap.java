...
/**
 * Decorates another <code>Map</code> returning a default value if the map does
 * not contain the requested key.
 * <p>
 * When the {@link #get(Object)} method is called with a key that does not exist
 * in the map, this map will return the default value specified in the
 * constructor/factory. Only the get method is altered, so the
 * {@link Map#containsKey(Object)} can be used to determine if a key really is
 * in the map or not.
 * <p>
 * The defaulted value is not added to the map. Compare this behaviour with
 * {@link LazyMap}, which does add the value to the map (via a Transformer).
 * <p>
 * For instance:
 * 
 * <pre>
 ...
 * <p>
 * <strong>Note that DefaultedMap is not synchronized and is not
...
public class DefaultedMap<K, V> extends AbstractMapDecorator<K, V> implements Serializable {
	 ...
	 * Factory method to create a defaulting map.
	 * <p>
	 * The factory specified is called when a missing key is found. The result will
	 * be returned as the result of the map get(key) method.
	 *
	 * @param <K>
	 *            the key type
	 * @param <V>
	 ...
	 * Factory method to create a defaulting map.
	 * <p>
	 * The transformer specified is called when a missing key is found. The key is
	 * passed to the transformer as the input, and the result will be returned as
	 ...
	 *            the map to decorate, must not be null
	 * @param transformer
	 *            the transformer to use as a factory to create entries, must not be
	 *            null
	 * @return a new defaulting map
	 * @throws NullPointerException
	 *             if map or factory is null
	...
	// -----------------------------------------------------------------------
	/**
	 * Constructs a new empty <code>DefaultedMap</code> that decorates a
	 * <code>HashMap</code>.
	 * <p>
	 * The object passed in will be returned by the map whenever an unknown key is
	 * requested.
	 *
	 * @param defaultValue
	 *            the default value to return when the key is not found
