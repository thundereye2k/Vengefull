 ...
 * extends clause will be removed in v5.0.
 *
 * @since 3.0
...
public class AbstractHashedMap<K, V> extends AbstractMap<K, V> implements IterableMap<K, V> {
	...
	/**
	 * Constructs a new, empty map with the specified initial capacity and default
	 * load factor.
	 *
	 * @param initialCapacity
	 ...
	 * {@link #put(Object, Object)}.
	 * <p>
	 * It is private to allow the constructor to still call it even when putAll is
	 * overriden.
	 *
	 * @param map
	 *            the map to add
	 ...
	 * implementation masks nulls. Subclasses can override this to perform alternate
	 * key conversions.
	 * <p>
	 ...
	 * implementation uses the equals method and assumes neither key is null.
	 ...
	 * <p>
	 * This implementation calls <code>setValue()</code> on the entry. Subclasses
	 ...
	 * <p>
	 * This implementation sets all the data fields on the entry. Subclasses could
	 * populate additional entry fields.
	 *
	 * @param entry
	 ...
	 * and <code>checkCapacity()</code>. It also handles changes to
	 * <code>modCount</code> and <code>size</code>. Subclasses could override to
	 * fully control adds to the map.
	 *
	 * @param hashIndex
	 ...
	 * <p>
	 * This implementation creates a new HashEntry instance. Subclasses can override
	 * this to return a different storage class, or implement caching.
	 *
	 * @param next
	 ...
	 * <code>destroyEntry()</code>. It also handles changes to <code>modCount</code>
	 * and <code>size</code>. Subclasses could override to fully control removals
	 * from the map.
	 *
	 * @param entry
	 *            the entry to remove
	 ...
	 * <p>
	 * This implementation removes the entry from the data storage table. The size
	 * is not updated. Subclasses could override to handle changes to the map.
	 *
	 * @param entry
	 *            the entry to remove
	 * @param hashIndex
	 ...
	 * <p>
	 * This implementation prepares the HashEntry for garbage collection. Subclasses
	 * can override this to implement caching (override clear as well).
	 *
	 * @param entry
	 ...
	 *
	 * @param newCapacity
	 *            the new capacity of the array (a power of two, less or equal to
	 *            max)
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Gets the <code>hashCode</code> field from a <code>HashEntry</code>. Used in
	 * subclasses that have no visibility of the field.
	 *
	 * @param entry
	 *            the entry to query, must not be null
	 ...
	 * methods to get the key and value, and set the value. It avoids the need to
	 * create an entrySet/keySet/values object. It also avoids creating the
	 * Map.Entry object.
	 *
	...
	// -----------------------------------------------------------------------
	/**
	 * Gets the keySet view of the map. Changes made to the view affect this map. To
	...
	/**
	 * Creates a key set iterator. Subclasses can override this to return iterators
	...
	// -----------------------------------------------------------------------
	/**
	 * Gets the values view of the map. Changes made to the view affect this map. To
	...
	/**
	 * Creates a values iterator. Subclasses can override this to return iterators
	 * with different properties.
	 *
	 ...
	 * then you will not be able to access the protected fields. The
	 * <code>entryXxx()</code> methods on <code>AbstractHashedMap</code> exist to
	 ...
	 * will initialise the superclass before the subclass. Sometimes however, this
	 * isn't what you want, as in this case the <code>put()</code> method on read
	 * can be affected by subclass state.
	 * <p>
	 * The solution adopted here is to serialize the state data of this class in
	 * this protected method. This method must be called by the
	 * <code>writeObject()</code> of the first serializable subclass.
	 * <p>
	 * Subclasses may override if they have a specific field that must be present on
	 * read before this implementation will work. Generally, the read determines
	 ...
	 * present before <code>put()</code> or <code>calculateThreshold()</code> will
	 * work correctly.
	 *
	 * @param in
	 *            the input stream
	 ...
	 *             if {@link AbstractMap#clone()} failed
	 */
	@Override
	@SuppressWarnings("unchecked")
