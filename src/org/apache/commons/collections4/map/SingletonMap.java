 ...
 * <p>
 * The single key/value pair is specified at creation. The map is fixed size so
 * any action that would change the size is disallowed. However, the
 * <code>put</code> or <code>setValue</code> methods can <i>change</i> the value
 * associated with the key.
 * <p>
 * If trying to remove or clear the map, an UnsupportedOperationException is
 * thrown. If trying to put a new mapping into the map, an
 * IllegalArgumentException is thrown. The put method will only succeed if the
 * key specified is the same as the singleton key.
 * <p>
...
public class SingletonMap<K, V> implements OrderedMap<K, V>, BoundedMap<K, V>, KeyValue<K, V>, Serializable, Cloneable {
	 ...
	 * key.
	 * <p>
	 * An IllegalArgumentException is thrown if the key does not match as the map is
	 * fixed size.
	 *
	 * @param key
	 ...
	 * <p>
	 * The map must be of size 0 or size 1. If it is size 1, the key must match the
	 * key of this map otherwise an IllegalArgumentException is thrown.
	 *
	 * @param map
	...
	// -----------------------------------------------------------------------
	/**
	 * Gets the entrySet view of the map. Changes made via <code>setValue</code>
	 * affect this map. To simply iterate through the entries, use
	 * {@link #mapIterator()}.
	 *
	...
	/**
	 * Gets the unmodifiable keySet view of the map. Changes made to the view affect
	 * this map. To simply iterate through the keys, use {@link #mapIterator()}.
	 *
	...
	/**
	 * Gets the unmodifiable values view of the map. Changes made to the view affect
	 * this map. To simply iterate through the values, use {@link #mapIterator()}.
	 *
	...
	/**
	 * Values implementation for the SingletonMap. This class is needed as values is
	 * a view that must update as the map updates.
	 */
	...
	@Override
	public String toString() {
		return new StringBuilder(128).append('{').append(getKey() == this ? "(this Map)" : getKey()).append('=')
				.append(getValue() == this ? "(this Map)" : getValue()).append('}').toString();
	}
