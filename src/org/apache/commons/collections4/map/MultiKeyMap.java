 ...
 * The best way to use this class is via the additional map-style methods. These
 * provide <code>get</code>, <code>containsKey</code>, <code>put</code> and
 * <code>remove</code> for individual keys which operate without extra object
 * creation.
 * <p>
 * The additional methods are the main interface of this map. As such, you will
 * not normally hold this map in a variable of type <code>Map</code>.
 * <p>
 * The normal map methods take in and return a {@link MultiKey}. If you try to
 * use <code>put()</code> with any other object type a
 * <code>ClassCastException</code> is thrown. If you try to use
 * <code>null</code> as the key in <code>put()</code> a
 * <code>NullPointerException</code> is thrown.
 * <p>
 * This map is implemented as a decorator of a <code>AbstractHashedMap</code>
 * which enables extra behaviour to be added easily.
 * <ul>
 * <li><code>MultiKeyMap.decorate(new LinkedMap())</code> creates an ordered
 * map.
 * <li><code>MultiKeyMap.decorate(new LRUMap())</code> creates an least recently
 * used map.
 * <li><code>MultiKeyMap.decorate(new ReferenceMap())</code> creates a garbage
 * collector sensitive map.
 * </ul>
 * Note that <code>IdentityMap</code> and <code>ReferenceIdentityMap</code> are
 * unsuitable for use as the key comparison would work on the whole MultiKey,
 * not the elements within.
 * <p>
 * As an example, consider a least recently used cache that uses a String
 * airline code and a Locale to lookup the airline's name:
 * 
 * <pre>
 ...
 * </pre>
 * <p>
 * <strong>Note that MultiKeyMap is not synchronized and is not
...
public class MultiKeyMap<K, V> extends AbstractMapDecorator<MultiKey<? extends K>, V>
	...
	// -----------------------------------------------------------------------
	/**
	 * Decorates the specified map to add the MultiKeyMap API and fast query. The
	 * map must not be null and must be empty.
	 *
	 * @param <K>
	 *            the key type
	 * @param <V>
	 ...
	 * {@link #multiKeyMap(AbstractHashedMap)}. The map must not be null and should
	 * be empty or only contain valid keys. This constructor performs no validation.
	 *
	 * @param map
	 *            the map to decorate
	 */
	...
	public V get(final Object key1, final Object key2) {
		...
		AbstractHashedMap.HashEntry<MultiKey<? extends K>, V> entry = decorated().data[decorated().hashIndex(hashCode,
				decorated().data.length)];
		while (entry != null) {
	 ...
	 */
	protected boolean isEqualKey(final AbstractHashedMap.HashEntry<MultiKey<? extends K>, V> entry, final Object key1,
			final Object key2) {
		final MultiKey<? extends K> multi = entry.getKey();
		return multi.size() == 2 && (key1 == multi.getKey(0) || key1 != null && key1.equals(multi.getKey(0)))
	...
	protected boolean isEqualKey(final AbstractHashedMap.HashEntry<MultiKey<? extends K>, V> entry, final Object key1,
		...
		final MultiKey<? extends K> multi = entry.getKey();
		return multi.size() == 3 && (key1 == multi.getKey(0) || key1 != null && key1.equals(multi.getKey(0)))
				&& (key2 == multi.getKey(1) || key2 != null && key2.equals(multi.getKey(1)))
	 ...
	 */
	protected boolean isEqualKey(final AbstractHashedMap.HashEntry<MultiKey<? extends K>, V> entry, final Object key1,
			final Object key2, final Object key3, final Object key4) {
		final MultiKey<? extends K> multi = entry.getKey();
		return multi.size() == 4 && (key1 == multi.getKey(0) || key1 != null && key1.equals(multi.getKey(0)))
				&& (key2 == multi.getKey(1) || key2 != null && key2.equals(multi.getKey(1)))
				&& (key3 == multi.getKey(2) || key3 != null && key3.equals(multi.getKey(2)))
	 ...
	 */
	protected boolean isEqualKey(final AbstractHashedMap.HashEntry<MultiKey<? extends K>, V> entry, final Object key1,
			final Object key2, final Object key3, final Object key4, final Object key5) {
		final MultiKey<? extends K> multi = entry.getKey();
		return multi.size() == 5 && (key1 == multi.getKey(0) || key1 != null && key1.equals(multi.getKey(0)))
				&& (key2 == multi.getKey(1) || key2 != null && key2.equals(multi.getKey(1)))
				&& (key3 == multi.getKey(2) || key3 != null && key3.equals(multi.getKey(2)))
				&& (key4 == multi.getKey(3) || key4 != null && key4.equals(multi.getKey(3)))
	 ...
	 * <p>
	 * This method removes all the mappings where the <code>MultiKey</code> has one
	 * or more keys, and the first matches that specified.
	 *
	 * @param key1
	 *            the first key
	 ...
	 * <p>
	 * This method removes all the mappings where the <code>MultiKey</code> has two
	 * or more keys, and the first two match those specified.
	 *
	 * @param key1
	 *            the first key
	 * @param key2
	 ...
	 * <p>
	 * This method removes all the mappings where the <code>MultiKey</code> has
	 * three or more keys, and the first three match those specified.
	 *
	 * @param key1
	 *            the first key
	 * @param key2
	 ...
	 * <p>
	 * This method removes all the mappings where the <code>MultiKey</code> has four
	 * or more keys, and the first four match those specified.
	 *
	 * @param key1
	 *            the first key
	 * @param key2
	...
	/**
	 * Copies all of the keys and values from the specified map to this map. Each
	 * key must be non-null and a MultiKey object.
	 *
	 * @param mapToCopy
	 *            to this map
	 * @throws NullPointerException
