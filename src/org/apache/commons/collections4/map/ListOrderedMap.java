 ...
 * using a <code>ListCommand</code> to maintain order.
 * <p>
 * The order will be used via the iterators and toArray methods on the views.
 * The order is also returned by the <code>MapIterator</code>. The
 * <code>orderedMapIterator()</code> method accesses an iterator that can
 * iterate both forwards and backwards through the map. In addition,
 * non-interface methods are provided to access the map by index.
 * <p>
 * If an object is added to the Map for a second time, it will remain in the
 * original position in the iteration.
 * <p>
 * <strong>Note that ListOrderedMap is not synchronized and is not
 ...
 * {@link CaseInsensitiveMap}, or similar maps that violate the general contract
 * of {@link java.util.Map}.</strong> The <code>ListOrderedMap</code> (or, more
 * precisely, the underlying <code>ListCommand</code>) is relying on
 ...
 * equals()}, and {@link Object#hashCode() hashCode()}, which
 * {@link java.util.IdentityHashMap IdentityHashMap}, and
 * {@link CaseInsensitiveMap} don't: The former uses <code>==</code>, and the
 * latter uses {@link Object#equals(Object) equals()} on a lower-cased key.
 * <p>
...
public class ListOrderedMap<K, V> extends AbstractMapDecorator<K, V> implements OrderedMap<K, V>, Serializable {
	...
	// -----------------------------------------------------------------------
	/**
	 * Constructs a new empty <code>ListOrderedMap</code> that decorates a
	 * <code>HashMap</code>.
	 *
	 * @since 3.1
	 */
	...
	/**
	 * Gets the next key to the one specified using insert order. This method
	 * performs a list search to find the key and is O(n).
	 *
	 * @param key
	...
	/**
	 * Gets the previous key to the one specified using insert order. This method
	 * performs a list search to find the key and is O(n).
	 *
	 * @param key
	 *            the key to find previous for
	...
	/**
	 * Puts the values contained in a supplied Map into the Map starting at the
	 * specified index.
	 *
	 * @param index
	 *            the index in the Map to start at.
	 * @param map
	...
	public void putAll(int index, final Map<? extends K, ? extends V> map) {
		...
		for (final Map.Entry<? extends K, ? extends V> entry : map.entrySet()) {
			...
			// The return value of put is null if the key did not exist OR the value was
			// null
	...
	/**
	 * Gets a view over the keys in the map as a ListCommand.
	 * <p>
	 * The ListCommand will be ordered by object insertion into the map. The ListCommand is
	 * unmodifiable.
	 *
	 * @see #keySet()
	...
	/**
	 * Gets a view over the values in the map as a ListCommand.
	 * <p>
	 * The ListCommand will be ordered by object insertion into the map. The ListCommand supports
	 * remove and set, but does not support add.
	 *
	 * @see #values()
	 ...
	 * <p>
	 * If the map already contains the key, then the original mapping is removed and
	 * the new mapping added at the specified index. The remove may change the
	 * effect of the index. The index is always calculated relative to the original
	 * state of the map.
	 * <p>
	 * Thus the steps are: (1) remove the existing key-value mapping, then (2)
	 * insert the new key-value mapping at the position it would have been inserted
	 * had the remove not occurred.
	 *
	 * @param index
	 *            the index at which the mapping should be inserted
	 * @param key
	 *            the key
	 * @param value
	 *            the value
	...
	/**
	 * Gets an unmodifiable ListCommand view of the keys which changes as the map changes.
	 * <p>
	 * The returned list is unmodifiable because changes to the values of the list
	 * (using {@link java.util.ListIterator#set(Object)}) will effectively remove
	 * the value from the list and reinsert that value at the end of the list, which
	 * is an unexpected side effect of changing the value of a list. This occurs
	 * because changing the key, changes when the mapping is added to the map and
	 * thus where it appears in the list.
	 * <p>
	 * An alternative to this method is to use the better named {@link #keyList()}
	 * or {@link #keySet()}.
	 *
	 * @see #keyList()
	 * @see #keySet()
