 ...
 * <p>
 * A somewhat subtle ramification of the least recently used algorithm is that
 * calls to {@link #get(Object)} stand a very good chance of modifying the map's
 * iteration order and thus invalidating any iterators currently in use. It is
 * therefore suggested that iterations over an {@link LRUMap} instance access
 * entry values only through a
 * {@link org.apache.commons.collections4.MapIterator MapIterator} or
 * {@link #entrySet()} iterator.
 * <p>
...
public class LRUMap<K, V> extends AbstractLinkedMap<K, V> implements BoundedMap<K, V>, Serializable, Cloneable {
	 ...
	 *             if the maximum size is less than one
	 * @throws IllegalArgumentException
	 *             if the initial size is negative or larger than the maximum size
	 * @since 4.1
	 */
	 ...
	 * Gets the value mapped to the key specified.
	 * <p>
	 * If {@code updateToMRU} is {@code true}, the position of the key in the map is
	 * changed to the most recently used position (last), otherwise the iteration
	 * order is not changed by this operation.
	 *
	 * @param key
	 *            the key
	 * @param updateToMRU
	 *            whether the key shall be updated to the most recently used
	 *            position
	 * @return the mapped value, null if no match
	 * @since 4.1
	 */
	 ...
	 * <p>
	 * This implementation moves the updated entry to the end of the list using
	 ...
	 * <p>
	 * This implementation checks the LRU size and determines whether to discard an
	 * entry or not using {@link #removeLRU(AbstractLinkedMap.LinkEntry)}.
	 * <p>
	 * From Commons Collections 3.1 this method uses {@link #isFull()} rather than
	 * accessing <code>size</code> and <code>maxSize</code> directly. It also
	 * handles the scanUntilRemovable functionality.
	 *
	 * @param hashIndex
	...
	protected void addMapping(final int hashIndex, final int hashCode, final K key, final V value) {
		if (isFull()) {
			...
			if (scanUntilRemovable) {
				...
				if (reuse == null) {
					throw new IllegalStateException("Entry.after=null, header.after" + header.after + " header.before"
			...
			if (removeLRUEntry) {
				if (reuse == null) {
					throw new IllegalStateException("reuse=null, header.after=" + header.after + " header.before"
							+ header.before + " key=" + key + " value=" + value + " size=" + size + " maxSize="
							+ maxSize
							+ " Please check that your keys are immutable, and that you have used synchronization properly."
							+ " If so, then please report this to dev@commons.apache.org as a bug.");
	...
	protected void reuseMapping(final LinkEntry<K, V> entry, final int hashIndex, final int hashCode, final K key,
		...
		try {
			...
			if (loop == null) {
				throw new IllegalStateException("Entry.next=null, data[removeIndex]=" + data[removeIndex] + " previous="
						+ previous + " key=" + key + " value=" + value + " size=" + size + " maxSize=" + maxSize
						+ " Please check that your keys are immutable, and that you have used synchronization properly."
						+ " If so, then please report this to dev@commons.apache.org as a bug.");
			}
			...
			throw new IllegalStateException("NPE, entry=" + entry + " entryIsHeader=" + (entry == header) + " key="
					+ key + " value=" + value + " size=" + size + " maxSize=" + maxSize
					+ " Please check that your keys are immutable, and that you have used synchronization properly."
					+ " If so, then please report this to dev@commons.apache.org as a bug.");
		}
	}
	...
	/**
	 * Subclass method to control removal of the least recently used entry from the
	 * map.
	 * <p>
	 * This method exists for subclasses to override. A subclass may wish to provide
	 * cleanup of resources when an entry is removed. For example:
	 * 
	 * <pre>
	 ...
	 * </pre>
	 * 
	 * The effect of returning false is dependent on the scanUntilRemovable flag. If
	 * the flag is true, the next LRU entry will be passed to this method and so on
	 * until one returns false and is removed, or every entry in the map has been
	 * passed. If the scanUntilRemovable flag is false, the map will exceed the
	 * maximum size.
	 * <p>
	 * NOTE: Commons Collections 3.0 passed the wrong entry to this method. This is
	 * fixed in version 3.1 onwards.
	 *
	 * @param entry
	...
	/**
	 * Whether this LRUMap will scan until a removable entry is found when the map
	 * is full.
	 *
