...
public class PredicateUtils {
	...
	/**
	 * Gets a Predicate that always throws an exception. This could be useful during
	...
	/**
	 * Creates a Predicate that checks if the input object is equal to the specified
	 * object using equals().
	 *
	 * @param <T>
	 *            the type that the predicate queries
	...
	/**
	 * Creates a Predicate that checks if the object passed in is of a particular
	 * type, using instanceof. A <code>null</code> input object will return
	 * <code>false</code>.
	 *
	 * @param type
	 ...
	 * encountered, and false if the same object is received again. The comparison
	 * is by equals(). A <code>null</code> input object is accepted and will return
	 * true the first time, and false subsequently as well.
	 *
	 * @param <T>
	 *            the type that the predicate queries
	 * @return the predicate
	...
	/**
	 * Creates a Predicate that invokes a method on the input object. The method
	 * must return either a boolean or a non-null Boolean, and have no parameters.
	 * If the input object is null, a PredicateException is thrown.
	 * <p>
	 * For example, <code>PredicateUtils.invokerPredicate("isEmpty");</code> will
	 * call the <code>isEmpty</code> method on the input object to determine the
	 * predicate result.
	 *
	 * @param <T>
	 *            the type that the predicate queries
	...
	public static <T> Predicate<T> invokerPredicate(final String methodName) {
		// reuse transformer as it has caching - this is lazy really, should have inner
		// class here
	 ...
	 *            the type that the predicate queries
	 * @param predicate1
	 *            the first predicate, may not be null
	 * @param predicate2
	 ...
	 * predicates are true. If the array of predicates is empty, then this predicate
	 * returns true.
	 *
	 * @param <T>
	 *            the type that the predicate queries
	...
	/**
	 * Create a new Predicate that returns true only if all of the specified
	 * predicates are true. The predicates are checked in iterator order. If the
	 * collection of predicates is empty, then this predicate returns true.
	 *
	 * @param <T>
	 *            the type that the predicate queries
	 * @param predicates
	...
	/**
	 * Create a new Predicate that returns true if any of the specified predicates
	 * are true. If the array of predicates is empty, then this predicate returns
	 * false.
	 *
	 * @param <T>
	 *            the type that the predicate queries
	...
	/**
	 * Create a new Predicate that returns true if any of the specified predicates
	 * are true. The predicates are checked in iterator order. If the collection of
	 ...
	 * Create a new Predicate that returns true if only one of the specified
	 * predicates are true. The predicates are checked in iterator order. If the
	 * collection of predicates is empty, then this predicate returns false.
	 *
	 * @param <T>
	 *            the type that the predicate queries
	 * @param predicates
	...
	/**
	 * Create a new Predicate that returns true if none of the specified predicates
	 * are true. If the array of predicates is empty, then this predicate returns
	 * true.
	 *
	 * @param <T>
	...
	/**
	 * Create a new Predicate that returns true if the specified predicate returns
	...
	/**
	 * Create a new Predicate that wraps a Transformer. The Transformer must return
	 * either Boolean.TRUE or Boolean.FALSE otherwise a PredicateException will be
	 * thrown.
	 *
	 * @param <T>
	 *            the type that the predicate queries
	...
	/**
	 * Gets a Predicate that returns false if the input object is null, otherwise it
	 * calls the specified Predicate. This allows null handling behaviour to be
	 * added to Predicates that don't support nulls.
	 *
	 * @param <T>
	 *            the type that the predicate queries
	 * @param predicate
	...
	/**
	 * Gets a Predicate that returns true if the input object is null, otherwise it
	 * calls the specified Predicate. This allows null handling behaviour to be
	 * added to Predicates that don't support nulls.
	 *
	 * @param <T>
	...
	// -----------------------------------------------------------------------
	/**
	 * Creates a predicate that transforms the input object before passing it to the
	 * predicate.
	 *
	 * @param <T>
	 *            the type that the predicate queries
