 ...
 * </ul>
 * <p>
 * Since v4.1 only closures which are considered to be unsafe are Serializable.
 * Closures considered to be unsafe for serialization are:
 * <ul>
 * <li>Invoker
 * <li>For
 * <li>While
...
public class ClosureUtils {
	...
	/**
	 * Gets a Closure that always throws an exception. This could be useful during
	...
	/**
	 * Gets a Closure that will do nothing. This could be useful during testing as a
	 * placeholder.
	 *
	...
	/**
	 * Creates a Closure that calls a Transformer each time it is called. The
	 * transformer will be called using the closure's input object. The
	 * transformer's result will be ignored.
	 *
	...
	/**
	 * Creates a Closure that will call the closure repeatedly until the predicate
	 * returns false.
	 *
	...
	/**
	 * Creates a Closure that will call the closure once and then repeatedly until
	...
	/**
	 * Creates a Closure that will invoke a specific method on the closure's input
	...
	/**
	 * Create a new Closure that calls each closure in turn, passing the result into
	 * the next closure.
	 *
	...
	/**
	 * Create a new Closure that calls each closure in turn, passing the result into
	 * the next closure. The ordering is that of the iterator() method on the
	 * collection.
	 *
	 * @see org.apache.commons.collections4.functors.ChainedClosure
	 *
	...
	/**
	 * Create a new Closure that calls another closure based on the result of the
	 * specified predicate.
	 *
	...
	/**
	 * Create a new Closure that calls one of two closures depending on the
	 * specified predicate.
	 *
	 * @see org.apache.commons.collections4.functors.IfClosure
	 *
	...
	/**
	 * Create a new Closure that calls one of the closures depending on the
	 * predicates.
	 * <p>
	 * The closure at array location 0 is called if the predicate at array location
	 * 0 returned true. Each predicate is evaluated until one returns true.
	 *
	 ...
	 * <p>
	 * The closure at array location 0 is called if the predicate at array location
	 * 0 returned true. Each predicate is evaluated until one returns true. If no
	 * predicates evaluate to true, the default closure is called.
	 *
	 * @see org.apache.commons.collections4.functors.SwitchClosure
	 *
	 * @param <E>
	 ...
	 * predicates.
	 * <p>
	 * The Map consists of Predicate keys and Closure values. A closure is called if
	 * its matching predicate returns true. Each predicate is evaluated until one
	 * returns true. If no predicates evaluate to true, the default closure is
	 * called. The default closure is set in the map with a null key. The ordering
	 * is that of the iterator() method on the entryset collection of the map.
	 *
	 * @see org.apache.commons.collections4.functors.SwitchClosure
	 *
	 * @param <E>
	...
	/**
	 * Create a new Closure that uses the input object as a key to find the closure
	 * to call.
	 * <p>
	 * The Map consists of object keys and Closure values. A closure is called if
	 * the input object equals the key. If there is no match, the default closure is
	 * called. The default closure is set in the map using a null key.
	 *
	 * @see org.apache.commons.collections4.functors.SwitchClosure
	 *
	 * @param <E>
