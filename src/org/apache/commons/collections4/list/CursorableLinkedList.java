...
/**
 * A <code>ListCommand</code> implementation with a <code>ListIterator</code> that
 * allows concurrent modifications to the underlying list.
 * <p>
 * This implementation supports all of the optional {@link ListCommand} operations. It
 * extends <code>AbstractLinkedList</code> and thus provides the
 * stack/queue/dequeue operations available in {@link java.util.LinkedList}.
 * <p>
 * The main feature of this class is the ability to modify the list and the
 * iterator at the same time. Both the {@link #listIterator()} and
 * {@link #cursor()} methods provides access to a <code>Cursor</code> instance
 * which extends <code>ListIterator</code>. The cursor allows changes to the
 * list concurrent with changes to the iterator. Note that the
 * {@link #iterator()} method and sublists do <b>not</b> provide this cursor
 * behaviour.
 * <p>
 ...
 * <code>WeakReference</code>. For most purposes, simply modify the iterator and
 * list at will, and then let the garbage collector to the rest.
 * <p>
...
public class CursorableLinkedList<E> extends AbstractLinkedList<E> implements Serializable {
	 ...
	 * <p>
	 * If the underlying list is modified while iterating using this iterator a
	 * ConcurrentModificationException will occur. The cursor behaviour is available
	 * via {@link #listIterator()}.
	 *
	...
	/**
	 * Returns a cursor iterator that allows changes to the underlying list in
	 * parallel.
	 * <p>
	 ...
	 * change in the list.
	 * <p>
	 * When the "current" (i.e., last returned by {@link ListIterator#next} or
	 * {@link ListIterator#previous}) element of the list is removed, the cursor
	 * automatically adjusts to the change (invalidating the last returned value
	 ...
	 * be cast to a <code>Cursor</code> if the <code>close</code> method is
	 * required.
	 *
	 * @return a new cursor iterator
	 */
	 ...
	 * such that it cannot be removed).
	 * <p>
	 * The {@link #listIterator(int)} method returns the same as this method, and
	 * can be cast to a <code>Cursor</code> if the <code>close</code> method is
	 * required.
	 *
	 * @param fromIndex
	 *            the index to start from
	...
	// -----------------------------------------------------------------------
	/**
	 * Updates the node with a new value. This implementation sets the value on the
	 * node. Subclasses can override this to record the change.
	 *
	 * @param node
	 *            node to update
	 * @param value
	...
	/**
	 * Informs all of my registered cursors that the specified element was just
	...
	// -----------------------------------------------------------------------
	/**
	 * An extended <code>ListIterator</code> that allows concurrent changes to the
	 * underlying list.
	 */
	public static class Cursor<E> extends AbstractLinkedList.LinkedListIterator<E> {
		 ...
		 * <p>
		 * There may have been subsequent alterations to the list since you obtained
		 * this item, however you can still remove it. You can even remove it if the
		 * item is no longer in the main list. However, you can't call this method on
		 * the same iterator more than once without calling next() or previous().
		 *
		...
		/**
		 * Adds an object to the list. The object added here will be the new 'previous'
		 * in the iterator.
		 *
		 * @param obj
		 *            the object to add
		...
		/**
		 * Mark this cursor as no longer being needed. Any resources associated with
		 * this cursor are immediately released. In previous versions of this class, it
		 * was mandatory to close all cursor objects to avoid memory leaks. It is <i>no
		 * longer</i> necessary to call this close method; an instance of this class can
		 * now be treated exactly like a normal iterator.
		 */
		public void close() {
			if (valid) {
