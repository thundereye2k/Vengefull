...
/**
 * A <code>ListCommand</code> implementation that is optimised for fast insertions and
...
public class TreeList<E> extends AbstractList<E> {
	 ...
	 * Iterator.
	 * <p>
	 * This method runs in O(n + log m) time, where m is the size of this list and n
	 * is the size of {@code c}.
	 *
	 * @param c
	...
	static class AVLNode<E> {
		 ...
		 * Constructs a new AVL tree from a collection.
		 * <p>
		 * This is a recursive helper for {@link #AVLNode(Collection)}. A call to this
		 * method will construct the subtree for elements {@code start} through
		 * {@code end} of the collection, assuming the iterator {@code e} already points
		 * at element {@code start}.
		 *
		 * @param iterator
		 *            an iterator over the collection, which should already point to the
		 *            element at index {@code start} within the collection
		 * @param start
		 *            the index of the first element in the collection that should be in
		 *            this subtree
		 * @param end
		 *            the index of the last element in the collection that should be in
		 *            this subtree
		 * @param absolutePositionOfParent
		 *            absolute position of this node's parent, or 0 if this node is the
		 *            root
		 * @param prev
		 *            the {@code AVLNode} corresponding to element (start - 1) of the
		 *            collection, or null if start is 0
		 * @param next
		 *            the {@code AVLNode} corresponding to element (end + 1) of the
		 *            collection, or null if end is the last element of the collection
		 */
		...
		public String toString() {
			return new StringBuilder().append("AVLNode(").append(relativePosition).append(',').append(left != null)
					.append(',').append(value).append(',').append(getRightSubTree() != null).append(", faedelung ")
					.append(rightIsNext).append(" )").toString();
		}
	}
	...
	static class TreeListIterator<E> implements ListIterator<E>, OrderedIterator<E> {
		...
		public void remove() {
			...
			// reset it now: will be retrieved by next call to next()/previous() via
			// nextIndex
			next = null;
			current = null;
			currentIndex = -1;
