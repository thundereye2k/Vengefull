...
/**
 * An abstract implementation of a linked list which provides numerous points
...
public abstract class AbstractLinkedList<E> implements List<E> {
	...
	/*
	 * Implementation notes: - a standard circular doubly-linked list - a marker
	 * node is stored to mark the start and the end of the list - node creation and
	 * removal always occurs through createNode() and removeNode(). - a modification
	 * count is kept, with the same semantics as {@link java.util.LinkedList}. -
	 * respects {@link AbstractList#modCount}
	 */

	/**
	 * A {@link Node} which indicates the start and end of the list and does not
	 * hold a value. The value of <code>next</code> is the first item in the list.
	 * The value of of <code>previous</code> is the last item in the list.
	 */
	 ...
	 * <p>
	 * If this constructor is used by a serializable subclass then the init() method
	 * must be called.
	 */
	...
	/**
	 * The equivalent of a default constructor, broken out so it can be called by
	 * any constructor and by <code>readObject</code>. Subclasses which override
	 * this method should make sure they call super, so the list is initialised
	 * properly.
	 */
	protected void init() {
	 ...
	 * {@inheritDoc}
	 * <p>
	 * This implementation iterates over the elements of this list, checking each
	 * element in turn to see if it's contained in <code>coll</code>. If it's
	 * contained, it's removed from this list. As a consequence, it is advised to
	 * use a collection type for <code>coll</code> that provides a fast (e.g. O(1))
	 ...
	 * <p>
	 * This implementation iterates over the elements of this list, checking each
	 * element in turn to see if it's contained in <code>coll</code>. If it's not
	 * contained, it's removed from this list. As a consequence, it is advised to
	 * use a collection type for <code>coll</code> that provides a fast (e.g. O(1))
	 * implementation of {@link Collection#contains(Object)}.
	 */
	...
	// -----------------------------------------------------------------------
	/**
	 * Compares two values for equals. This implementation uses the equals method.
	...
	/**
	 * Creates a new node with previous, next and element all set to null. This
	 * implementation creates a new empty Node. Subclasses can override this to
	...
	/**
	 * Creates a new node with the specified properties. This implementation creates
	 * a new Node with data. Subclasses can override this to create a different
	 * class.
	 *
	 * @param value
	...
	/**
	 * Creates a new node with the specified object as its <code>value</code> and
	 * inserts it before <code>node</code>.
	 * <p>
	...
	/**
	 * Creates a new node with the specified object as its <code>value</code> and
	 * inserts it after <code>node</code>.
	 * <p>
	 * This implementation uses {@link #createNode(Object)} and
	 * {@link #addNode(AbstractLinkedList.Node,AbstractLinkedList.Node)}.
	 *
	 ...
	 *            whether or not the end marker can be returned if startIndex is set
	 *            to the list's size
	 * @return the node at the given index
	 * @throws IndexOutOfBoundsException
	 *             if the index is less than 0; equal to the size of the list and
	 *             endMakerAllowed is false; or greater than the size of the list
	 */
	...
	protected static class LinkedListIterator<E> implements ListIterator<E>, OrderedIterator<E> {
		 ...
		 *             If {@link #next()} or {@link #previous()} haven't been called, or
		 *             if the node has been removed with {@link #remove()} or a new node
		 *             added with {@link #add(Object)}.
		 */
