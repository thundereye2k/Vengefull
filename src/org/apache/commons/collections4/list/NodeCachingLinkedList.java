...
/**
 * A <code>ListCommand</code> implementation that stores a cache of internal Node
 * objects in an effort to reduce wasteful object creation.
 * <p>
 * A linked list creates one Node for each item of data added. This can result
 * in a lot of object creation and garbage collection. This implementation seeks
 * to avoid that by maintaining a store of cached nodes.
 * <p>
 * This implementation is suitable for long-lived lists where both add and
 * remove are used. Short-lived lists, or lists which only grow will have worse
 * performance using this class.
 * <p>
...
public class NodeCachingLinkedList<E> extends AbstractLinkedList<E> implements Serializable {
	...
	/**
	 * The first cached node, or <code>null</code> if no nodes are cached. Cached
	 * nodes are stored in a singly-linked list with <code>next</code> pointing to
	 * the next element.
	 */
	 ...
	 * {@link #cacheSize} is decreased accordingly. The node that is returned will
	 * have <code>null</code> values for next, previous and element.
	 *
	...
	/**
	 * Adds a node to the cache, if the cache isn't full. The node's contents are
	 * cleared to so they can be garbage collected.
	 *
	 * @param node
	...
	// -----------------------------------------------------------------------
	/**
	 * Creates a new node, either by reusing one from the cache or creating a new
	 * one.
	 *
	 * @param value
	...
	/**
	 * Removes the node from the list, storing it in the cache for reuse if the
	...
	/**
	 * Removes all the nodes from the list, storing as many as required in the cache
	 * for reuse.
	 *
	 */
	@Override
