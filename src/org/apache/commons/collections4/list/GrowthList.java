...
/**
 * Decorates another <code>ListCommand</code> to make it seamlessly grow when indices
 * larger than the list size are used on add and set, avoiding most
 * IndexOutOfBoundsExceptions.
 * <p>
 * This class avoids errors by growing when a set or add method would normally
 * throw an IndexOutOfBoundsException. Note that IndexOutOfBoundsException IS
 * returned for invalid negative indices.
 * <p>
 * Trying to set or add to an index larger than the size will cause the list to
 * grow (using <code>null</code> elements). Clearly, care must be taken not to
 * use excessively large indices, as the internal list will grow to match.
 * <p>
 ...
 * IndexOutOfBoundsExceptions.
 * <p>
 * This class differs from {@link LazyList} because here growth occurs on set
 * and add, where <code>LazyList</code> grows on get. However, they can be used
 * together by decorating twice.
 *
 * @see LazyList
 * @since 3.2
...
public class GrowthList<E> extends AbstractSerializableListDecorator<E> {
	 ...
	 * <p>
	 * If the requested index is greater than the current size, the list will grow
	 * to the new size. Indices between the old size and the requested size will be
	 * filled with <code>null</code>.
	 * <p>
	 * If the index is less than the current size, the value will be added to the
	 * underlying list directly. If the index is less than zero, the underlying list
	 * is called, which will probably throw an IndexOutOfBoundsException.
	 *
	 * @param index
	 *            the index to add at
	 * @param element
	 ...
	 * filled with <code>null</code>.
	 * <p>
	 * If the index is less than the current size, the values will be added to the
	 * underlying list directly. If the index is less than zero, the underlying list
	 * is called, which will probably throw an IndexOutOfBoundsException.
	 *
	 * @param index
	 ...
	 * filled with <code>null</code>.
	 * <p>
	 * If the index is less than the current size, the value will be set onto the
	 * underlying list directly. If the index is less than zero, the underlying list
	 * is called, which will probably throw an IndexOutOfBoundsException.
	 *
	 * @param index
