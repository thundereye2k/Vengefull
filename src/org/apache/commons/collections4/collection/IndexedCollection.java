 ...
 *
 * @param <K>
 *            the type of object in the index.
 * @param <C>
...
public class IndexedCollection<K, C> extends AbstractCollectionDecorator<C> {
	 ...
	 * <p>
	 * If an element is added, which maps to an existing key, an
	 * {@link IllegalArgumentException} will be thrown.
	 *
	 * @param <K>
	 *            the index object type.
	 * @param <C>
	 *            the collection type.
	 * @param coll
	 *            the decorated {@link Collection}.
	...
	public static <K, C> IndexedCollection<K, C> uniqueIndexedCollection(final Collection<C> coll,
		...
		return new IndexedCollection<K, C>(coll, keyTransformer,
				MultiValueMap.<K, C>multiValueMap(new HashMap<K, Collection<C>>()), true);
	}
	...
	public static <K, C> IndexedCollection<K, C> nonUniqueIndexedCollection(final Collection<C> coll,
			final Transformer<C, K> keyTransformer) {
		return new IndexedCollection<K, C>(coll, keyTransformer,
				MultiValueMap.<K, C>multiValueMap(new HashMap<K, Collection<C>>()), false);
	}
	 ...
	 *
	 * @param coll
	 *            decorated {@link Collection}
	 * @param keyTransformer
	 *            {@link Transformer} for generating index keys
	 * @param map
	 *            map to use as index
	 * @param uniqueIndex
	 *            if the index shall enforce uniqueness of index keys
	 */
	public IndexedCollection(final Collection<C> coll, final Transformer<C, K> keyTransformer, final MultiMap<K, C> map,
	 ...
	 *             if the object maps to an existing key and the index enforces a
	 ...
	 * <p>
	 * In case of a non-unique index, this method will return the first value
	 * associated with the given key. To retrieve all elements associated with a
	 * key, use {@link #values(Object)}.
	 *
	 * @param key
	 *            key to look up
	 ...
	 * @param key
	 *            key to look up
	 * @return a collection of elements found, or null if
	 *         {@code contains(key) == false}
	 */
	@SuppressWarnings("unchecked") // index is a MultiMap which returns a Collection
