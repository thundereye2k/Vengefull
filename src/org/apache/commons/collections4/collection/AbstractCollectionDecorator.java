 ...
 * necessary to preserve the symmetry of equals. Custom definitions of equality
 * are usually based on an interface, such as Set or ListCommand, so that the
 * implementation of equals can cast the object being tested for equality to the
 * custom interface. AbstractCollectionDecorator does not implement such custom
 * interfaces directly; they are implemented only in subclasses. Therefore,
 * forwarding equals would break symmetry, as the forwarding object might
 * consider itself equal to the object being tested, but the reverse could not
 * be true. This behavior is consistent with the JDK's collection wrappers, such
 * as {@link java.util.Collections#unmodifiableCollection(Collection)}. Use an
 * interface-specific subclass of AbstractCollectionDecorator, such as
 ...
 * @since 3.0
 * @version $Id: AbstractCollectionDecorator.java 1686855 2015-06-22 13:00:27Z
 *          tn $
 */
public abstract class AbstractCollectionDecorator<E> implements Collection<E>, Serializable {
	...
	/**
	 * Gets the collection being decorated. All access to the decorated collection
	 * goes via this method.
	 *
