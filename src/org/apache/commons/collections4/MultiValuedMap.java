 ...
 * <ul>
 * <li>Putting a value into the map will add the value to a {@link Collection}
 * at that key.</li>
 * <li>Getting a value will return a {@link Collection}, holding all the values
 * put to that key.</li>
 * </ul>
 * <p>
 * For example:
...
public interface MultiValuedMap<K, V> {
	 ...
	 * <p>
	 * Implementations would return the total size of the map which is the count of
	 * the values from all keys.
	 *
	 ...
	 * mapping for a key {@code k} such that
	 * {@code (key==null ? k==null : key.equals(k))}. (There can be at most one such
	 * mapping.)
	 *
	 * @param key
	 ...
	 * @throws NullPointerException
	 *             if the value is null and null values are not supported by the
	 *             used collection types (optional)
	 */
	 ...
	 * {@link #containsKey(Object)} returns {@code false}. Changes to the returned
	 * collection will update the underlying {@code MultiValuedMap} and vice-versa.
	 *
	 * @param key
	 *            the key to retrieve
	 * @return the {@code Collection} of values, implementations should return an
	 *         empty collection for no mapping
	 * @throws NullPointerException
	 *             if the key is null and null keys are invalid (optional)
	 */
	 ...
	 * <p>
	 * Unlike a normal {@code Map} the previous value is not replaced. Instead the
	 * new value is added to the collection stored against the key. Depending on the
	 * collection type used, duplicate key-value mappings may be allowed.
	 * <p>
	 * The method will return {@code true} if the size of the multi-valued map has
	 * been increased because of this operation.
	 *
	 * @param key
	 ...
	 *             if the put operation is not supported by this multi-valued map,
	 *             e.g. if it is unmodifiable
	 * @throws NullPointerException
	 *             if the key or value is null and null is invalid (optional)
	 * @throws IllegalArgumentException
	 *             if some aspect of the specified key or value prevents it from
	 *             being stored in this multi-valued map
	 */
	boolean put(K key, V value);

	/**
	 * Adds a mapping to the specified key for all values contained in the given
	 * Iterable.
	 *
	 * @param key
	 *            the key to store against
	 * @param values
	 *            the values to add to the collection at the key, may not be null
	 * @return true if the map changed as a result of this operation
	 * @throws NullPointerException
	 *             if the specified iterable is null, or if this map does not permit
	 *             null keys or values, and the specified key or values contain null
	 *             (optional)
	 */
	boolean putAll(K key, Iterable<? extends V> values);

	/**
	 * Copies all mappings from the specified map to this multi-valued map (optional
	 * operation).
	 * <p>
	 ...
	 * <p>
	 * The returned collection <i>may</i> be modifiable, but updates will not be
	 * propagated to this multi-valued map. In case no mapping was stored for the
	 ...
	 * <p>
	 * If the last value for a key is removed, implementations typically return an
	 * empty collection from a subsequent <code>get(Object)</code>.
	 *
	 * @param key
	...
	/**
	 * Returns a {@link MultiSet} view of the keys contained in this multi-valued
	 * map.
	 * <p>
	 * The {@link MultiSet#getCount(Object)} method of the returned multiset will
	 * give the same result a calling {@code get(Object).size()} for the same key.
	 * <p>
	 * This multiset is backed by the map, so any changes in the map are reflected
	 * in the multiset.
	 *
	 ...
	 * through the iterator's own {@code remove} operation), the result of the
	 * iteration is undefined. The set supports element removal, which removes the
	...
	/**
	 * Gets a {@link Collection} view of all values contained in this multi-valued
	 * map.
	 * <p>
	 * Implementations typically return a collection containing the combination of
	 ...
	 * update the underlying multi-valued map, and vice versa. The map does not
	 ...
	 * to access the entries collection or use {@code Map.Entry} objects.
	 *
