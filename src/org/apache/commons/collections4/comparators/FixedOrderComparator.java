 ...
 * subsequent calls to {@link #compare(Object, Object) compare} yield that
 * order. For example:
 * 
 * <pre>
...
public class FixedOrderComparator<T> implements Comparator<T>, Serializable {
	...
	/**
	 * Constructs a FixedOrderComparator which uses the order of the given array to
	 * compare the objects.
	 * <p>
	...
	/**
	 * Constructs a FixedOrderComparator which uses the order of the given list to
	 * compare the objects.
	 * <p>
	 ...
	 *            the flag for unknown behaviour - UNKNOWN_AFTER, UNKNOWN_BEFORE or
	 *            UNKNOWN_THROW_EXCEPTION
	 * @throws UnsupportedOperationException
	 *             if a comparison has been performed
	 * @throws NullPointerException
	 *             if unknownObjectBehavior is null
	 */
	...
	// -----------------------------------------------------------------------
	/**
	 * Adds an item, which compares as after all items known to the Comparator. If
	 * the item is already known to the Comparator, its old position is replaced
	 * with the new position.
	 *
	 * @param obj
	 *            the item to be added to the Comparator.
	 * @return true if obj has been added for the first time, false if it was
	 *         already known to the Comparator.
	 * @throws UnsupportedOperationException
	 ...
	 *
	 * @param existingObj
	 *            an item already in the Comparator's set of known objects
	 * @param newObj
	 *            an item to be added to the Comparator's set of known objects
	 * @return true if newObj has been added for the first time, false if it was
	 *         already known to the Comparator.
	 * @throws IllegalArgumentException
	 *             if existingObject is not in the Comparator's set of known
	 *             objects.
	 * @throws UnsupportedOperationException
	 *             if a comparison has already been made
	 */
	 ...
	 * IllegalArgumentException in the case of an unrecognised object. This is not
	 * specified in the Comparator interface, but is the most appropriate exception.
	 *
	 * @param obj1
	 ...
	 * @throws IllegalArgumentException
	 *             if obj1 or obj2 are not known to this Comparator and an
	 *             alternative behavior has not been set via
	 *             {@link #setUnknownObjectBehavior(UnknownObjectBehavior)}.
	 */
	@Override
	 ...
	 * a {@link FixedOrderComparator} whose attributes are equal to mine.
	 *
	 * @param object
		...
		if (object.getClass().equals(this.getClass())) {
			...
			return (null == map ? null == comp.map : map.equals(comp.map))
					&& (null == unknownObjectBehavior ? null == comp.unknownObjectBehavior
							: unknownObjectBehavior == comp.unknownObjectBehavior && counter == comp.counter
