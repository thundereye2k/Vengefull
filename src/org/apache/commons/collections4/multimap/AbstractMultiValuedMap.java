...
public abstract class AbstractMultiValuedMap<K, V> implements MultiValuedMap<K, V> {
	 ...
	 *
	 * @param map
	 *            the map to wrap, must not be null
	...
	/**
	 * Gets the collection of values associated with the specified key. This would
	 * return an empty collection in case the mapping is not present
	 *
	 * @param key
	 *            the key to retrieve
	 * @return the {@code Collection} of values, will return an empty
	 *         {@code Collection} for no mapping
	 */
	@Override
	 ...
	 * @return the <code>Collection</code> of values removed, will return an empty,
	 *         unmodifiable collection for no mapping found
	 */
	@Override
	 ...
	 * <p>
	 * The value is removed from the collection mapped to the specified key. Other
	 * values attached to that key are unaffected.
	 * <p>
	 * If the last value for a key is removed, an empty collection would be returned
	 * from a subsequent {@link #get(Object)}.
	 *
	 * @param key
	 ...
	 * {@inheritDoc}
	 * <p>
	 * This implementation does <b>not</b> cache the total size of the multi-valued
	 * map, but rather calculates it by iterating over the entries of the underlying
	 * map.
	 */
	@Override
	public int size() {
	 ...
	 * this call is equivalent to that of calling {@link #put(Object,Object) put(k,
	 * v)} on this map once for each mapping from key {@code k} to value {@code v}
	 * in the specified map. The behavior of this operation is undefined if the
	...
	/**
	 * Copies all of the mappings from the specified MultiValuedMap to this map. The
	 ...
	 * <p>
	 * Returns a MultiSet of keys with its values count as the count of the
	 * MultiSet. This multiset is backed by the map, so any changes in the map is
	 * reflected here. Any method which modifies this multiset like {@code add},
	 * {@code remove}, {@link Iterator#remove()} etc throws
	...
	/**
	 * Wrapped collection to handle add and remove on the collection returned by
	 * get(object).
	 * <p>
	 * Currently, the wrapped collection is not cached and has to be retrieved from
	 * the underlying map. This is safe, but not very efficient and should be
	 * improved in subsequent releases. For this purpose, the scope of this
	 * collection is set to package private to simplify later refactoring.
	 */
	 ...
	 *             if the stream contains an object which class can not be loaded
