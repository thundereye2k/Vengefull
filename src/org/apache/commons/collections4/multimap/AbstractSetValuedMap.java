...
public abstract class AbstractSetValuedMap<K, V> extends AbstractMultiValuedMap<K, V> implements SetValuedMap<K, V> {
	...
	// -----------------------------------------------------------------------
	/**
	 * Gets the set of values associated with the specified key. This would return
	 * an empty set in case the mapping is not present
	 *
	 * @param key
	 *            the key to retrieve
	 * @return the <code>Set</code> of values, will return an empty <code>Set</code>
	 *         for no mapping
	 */
	@Override
