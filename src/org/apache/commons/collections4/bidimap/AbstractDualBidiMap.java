...
public abstract class AbstractDualBidiMap<K, V> implements BidiMap<K, V> {
	 ...
	 * <p>
	 * This constructor remains in place for deserialization. All other usage is
	 * deprecated in favour of {@link #AbstractDualBidiMap(Map, Map)}.
	 */
	 ...
	 * <p>
	 * The two maps must be a matching pair, normal and reverse. They will typically
	 * both be empty.
	 * <p>
	 * Neither map is validated, so nulls may be passed in. If you choose to do this
	 * then the subclass constructor must populate the <code>maps[]</code> instance
	 * variable itself.
	 *
	 * @param normalMap
	 ...
	 * <code>createBidiMap</code> implementation.
	 *
	 * @param normalMap
	 *            the normal direction map
	 * @param reverseMap
	...
	// -----------------------------------------------------------------------
	/**
	 * Obtains a <code>MapIterator</code> over the map. The iterator implements
	 * <code>ResetableMapIterator</code>. This implementation relies on the entrySet
	 * iterator.
	 * <p>
	 * The setValue() methods only allow a new value to be set. If the value being
	 * set is already in the map, an IllegalArgumentException is thrown (as setValue
	 * cannot change the size of the map).
	 *
	...
	// -----------------------------------------------------------------------
	/**
	 * Gets a keySet view of the map. Changes made on the view are reflected in the
	 * map. The set supports remove and clear but not add.
	 *
	...
	/**
	 * Gets a values view of the map. Changes made on the view are reflected in the
	 * map. The set supports remove and clear but not add.
	 *
	...
	/**
	 * Gets an entrySet view of the map. Changes made on the set are reflected in
	 * the map. The set supports remove and clear but not add.
	 * <p>
	 * The Map Entry setValue() method only allow a new value to be set. If the
	 * value being set is already in the map, an IllegalArgumentException is thrown
	...
	protected static abstract class View<K, V, E> extends AbstractCollectionDecorator<E> {
		 ...
		 *
		 * @param coll
		 *            the collection view being decorated
		 * @param parent
		 *            the parent BidiMap
		 */
		 ...
		 * {@inheritDoc}
		 * <p>
		 * This implementation iterates over the elements of this bidi map, checking
		 * each element in turn to see if it's contained in <code>coll</code>. If it's
		 * not contained, it's removed from this bidi map. As a consequence, it is
		 * advised to use a collection type for <code>coll</code> that provides a fast
