 ...
 * <p>
 * The setValue() method on iterators will succeed only if the new value being
 * set is not already in the bidimap.
 * <p>
 * When considering whether to use this class, the {@link TreeBidiMap} class
 * should also be considered. It implements the interface using a dedicated
 * design, and does not store each object twice, which can save on memory use.
 * <p>
...
public class DualTreeBidiMap<K, V> extends AbstractDualBidiMap<K, V> implements SortedBidiMap<K, V>, Serializable {
	 ...
	 * <p>
	 * This implementation copies the elements to an ArrayList in order to provide
	 * the forward/backward behaviour.
	 *
