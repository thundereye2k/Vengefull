 ...
 * ascending value order, sorted according to the natural order for the key's
 * and value's classes.
 * <p>
 ...
 * directed to the TreeMap that maps keys to values), there are problems with
 * that implementation. If the data contained in the TreeMaps is large, the cost
 * of redundant storage becomes significant. The {@link DualTreeBidiMap} and
 ...
 * red-black algorithm is based on {@link java.util.TreeMap}, but has been
 * modified to simultaneously map a tree node by key and by value. This doubles
 * the cost of put operations (but so does using two TreeMaps), and nearly
 * doubles the cost of remove operations (there is a savings in that the lookup
 * of the node to be removed only has to be performed once). And since only one
 * node contains the key and value, storage is significantly less than that
 * required by two TreeMaps.
 * <p>
 * The Map.Entry instances returned by the appropriate methods will not allow
 * setValue() and will throw an UnsupportedOperationException on attempts to
 * call that method.
 *
...
public class TreeBidiMap<K extends Comparable<K>, V extends Comparable<V>>
	 ...
	 *             if the keys/values in the map are not Comparable or are not
	 *             mutually comparable
	...
	/**
	 * Gets the value to which this map maps the specified key. Returns null if the
	 ...
	 * <p>
	 * When adding a key-value pair, the value may already exist in the map against
	 * a different key. That mapping is removed, to ensure that the value only
	...
	// -----------------------------------------------------------------------
	/**
	 * Returns the key to which this map maps the specified value. Returns null if
	 * the map contains no mapping for this value.
	 * <p>
	 * The value must implement <code>Comparable</code>.
	 *
	 * @param value
	 *            value whose associated key is to be returned.
	 * @return the key to which this map maps the specified value, or null if the
	 *         map contains no mapping for this value.
	 * @throws ClassCastException
	 *             if the value is of an inappropriate type
	 * @throws NullPointerException
	 *             if the value is null
	 ...
	 * @return previous key associated with specified value, or null if there was no
	 *         mapping for value.
	 * @throws ClassCastException
	 *             if the value is of an inappropriate type
	 * @throws NullPointerException
	...
	// -----------------------------------------------------------------------
	/**
	 * Returns a set view of the values contained in this map in key order. The
	 * returned object can be cast to a Set.
	 * <p>
	 * The set is backed by the map, so changes to the map are reflected in the set,
	 * and vice-versa. If the map is modified while an iteration over the set is in
	 * progress, the results of the iteration are undefined.
	...
	// -----------------------------------------------------------------------
	/**
	 * Returns a set view of the entries contained in this map in key order. For
	 * simple iteration through the map, the MapIterator is quicker.
	 * <p>
	 * The set is backed by the map, so changes to the map are reflected in the set,
	 * and vice-versa. If the map is modified while an iteration over the set is in
	 * progress, the results of the iteration are undefined.
	 * <p>
	 * The set supports element removal, which removes the corresponding mapping
	 * from the map. It does not support the add or addAll operations. The returned
	 ...
	 *
	 * @param key
	 *            the key, always the main map key
	 * @param value
	 ...
	 * @return the desired Node, or null if there is no mapping of the specified
	 *         data
	 */
	@SuppressWarnings("unchecked")
	 ...
	 *            the second object
	 *
	 * @return negative value if o1 &lt; o2; 0 if o1 == o2; positive value if o1
	 *         &gt; o2
	 */
	 ...
	 *
	 * @param node
	 *            the node from which we will start searching
	 * @param index
	 *            the KEY or VALUE int
	...
	/**
	 * copy the color from one node to another, dealing with the fact that one or
	 * both nodes may, in fact, be null
	 *
	 * @param from
	 *            the node whose color we're copying; may be null
	 * @param to
	...
	/**
	 * is the specified node red? if the node does not exist, no, it's black, thank
	 * you
	 *
	 * @param node
	...
	/**
	 * get a node's grandparent. mind you, the node, its parent, or its grandparent
	 * may not exist. no problem
	 *
	 * @param node
	 *            the node (may be null) in question
	 * @param index
	...
	/**
	 * get a node's parent. mind you, the node, or its parent, may not exist. no
	 * problem
	 *
	 * @param node
	 *            the node (may be null) in question
	...
	/**
	 * complicated red-black insert stuff. Based on Sun's TreeMap implementation,
	 * though it's barely recognizable any more
	 *
	 * @param insertedNode
	...
	private void doRedBlackInsert(final Node<K, V> insertedNode, final DataElement dataElement) {
		...
		while (currentNode != null && currentNode != rootNode[dataElement.ordinal()]
	...
	/**
	 * complicated red-black delete stuff. Based on Sun's TreeMap implementation,
	 * though it's barely recognizable any more
	 *
	 * @param deletedNode
	...
	private void doRedBlackDelete(final Node<K, V> deletedNode) {
		for (final DataElement dataElement : DataElement.values()) {
			...
			final Node<K, V> replacement = deletedNode.getLeft(dataElement) != null ? deletedNode.getLeft(dataElement)
	...
	/**
	 * complicated red-black delete stuff. Based on Sun's TreeMap implementation,
	 * though it's barely recognizable any more. This rebalances the tree (somewhat,
	 * as red-black trees are not perfectly balanced -- perfect balancing takes
	 * longer)
	 *
	...
	/**
	 * swap two nodes (except for their content), taking care of special cases where
	 * one is the other's parent ... hey, it happens.
	 *
	 * @param x
	 *            one node
	 * @param y
	...
	private void swapPosition(final Node<K, V> x, final Node<K, V> y, final DataElement dataElement) {
		...
		final boolean xWasLeftChild = x.getParent(dataElement) != null
				&& x == x.getParent(dataElement).getLeft(dataElement);
		final boolean yWasLeftChild = y.getParent(dataElement) != null
				&& y == y.getParent(dataElement).getLeft(dataElement);
	...
	/**
	 * check if an object is fit to be proper input ... has to be Comparable and
	 * non-null
	 *
	 * @param o
	 *            the object being checked
	 * @param index
	 *            the KEY or VALUE int (used to put the right word in the exception
	 *            message)
	 *
	 * @throws NullPointerException
	 *             if o is null
	...
	/**
	 * increment the modification count -- used to check for concurrent modification
	 * of the map through the map and through an Iterator from one of its Set or
	 * Collection views
	 */
	private void modify() {
		modifications++;
	...
	static class Node<K extends Comparable<K>, V extends Comparable<V>> implements Map.Entry<K, V>, KeyValue<K, V> {
		...
		/**
		 * Make a new cell with given key and value, and with null links, and black
		 * (true) colors.
		 *
		 * @param key
		 * @param value
		 ...
		 * the given object is also a map entry and the two entries represent the same
		 * mapping.
		 *
		 * @param obj
