 ...
 * which, by virtue of its implementing {@link Map}&lt;K, V&gt;, must be
 * constructed in such a way that its read and write parameters are generalized
 * to a common (super-)type. In practice this would often mean
 * <code>&gt;Object, Object&gt;</code>, defeating much of the usefulness of
 * having parameterized types.
 * <p>
 * On the downside, this class is not drop-in compatible with
 * {@link java.util.Map} but is intended to be worked with either directly or by
 * {@link Put} and {@link org.apache.commons.collections4.Get Get}
 * generalizations.
 *
 * @since 4.0
...
public class TransformedSplitMap<J, K, U, V> extends AbstractIterableGetMapDecorator<K, V>
	 ...
	 *            the transformer to use for key conversion, must not be null
	 * @param valueTransformer
	 *            the transformer to use for value conversion, must not be null
