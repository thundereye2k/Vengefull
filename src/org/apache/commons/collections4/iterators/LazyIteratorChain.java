...
/**
 * An LazyIteratorChain is an Iterator that wraps a number of Iterators in a
 * lazy manner.
 * <p>
 * This class makes multiple iterators look like one to the caller. When any
 * method from the Iterator interface is called, the LazyIteratorChain will
 * delegate to a single underlying Iterator. The LazyIteratorChain will invoke
 * the Iterators in sequence until all Iterators are exhausted.
 * <p>
 * The Iterators are provided by {@link #nextIterator(int)} which has to be
 * overridden by sub-classes and allows to lazily create the Iterators as they
 * are accessed:
 * 
 * <pre>
 ...
 * {@link #nextIterator(int)} will be called to obtain another iterator, and so
 * on until {@link #nextIterator(int)} returns null, indicating that the chain
 * is exhausted.
 * <p>
 * NOTE: The LazyIteratorChain may contain no iterators. In this case the class
...
public abstract class LazyIteratorChain<E> implements Iterator<E> {
	...
	/**
	 * The "last used" Iterator is the Iterator upon which next() or hasNext() was
	 * most recently called used for the remove() operation only.
	 */
	 ...
	 *
	 * @param count
	 *            the number of time this method has been called (starts with 1)
	 ...
	 * Iterator.
	 * <p>
	 * As with next() and hasNext(), this method calls remove() on the underlying
	 * Iterator. Therefore, this method may throw an UnsupportedOperationException
	 * if the underlying Iterator does not support this method.
	 *
