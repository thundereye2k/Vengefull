 ...
 * you have an object array, the {@link ObjectArrayListIterator} class is a
 * better choice, as it will perform better.
 *
 * <p>
 * This iterator does not support {@link #add(Object)} or {@link #remove()}, as
 * the array cannot be changed in size. The {@link #set(Object)} method is
 * supported however.
 *
...
public class ArrayListIterator<E> extends ArrayIterator<E> implements ResettableListIterator<E> {
	...
	/**
	 * Holds the index of the last item returned by a call to <code>next()</code> or
	 * <code>previous()</code>. This is set to <code>-1</code> if neither method has
	 * yet been invoked. <code>lastItemIndex</code> is used to to implement the
	 * {@link #set} method.
	 */
	...
	/**
	 * Construct an ArrayListIterator that will iterate over a range of values in
	 * the specified array.
	 *
	 * @param array
	 *            the array to iterate over
	...
	/**
	 * This iterator does not support modification of its backing collection, and so
	 * will always throw an {@link UnsupportedOperationException} when this method
	 * is invoked.
	 *
	 * @param o
	 *            the element to add
	 ...
	 * <p>
	 * This method sets the element that was returned by the last call to
	 * {@link #next()} of {@link #previous()}.
	 * <p>
	 * <b>Note:</b> {@link ListIterator} implementations that support
	 * <code>add()</code> and <code>remove()</code> only allow <code>set()</code> to
	 * be called once per call to <code>next()</code> or <code>previous</code> (see
	 * the {@link ListIterator} javadoc for more details). Since this implementation
	 * does not support <code>add()</code> or <code>remove()</code>,
	 * <code>set()</code> may be called as often as desired.
	 *
	 * @param o
	 *            the element to set
	 * @throws IllegalStateException
	 *             if {@link #next()} or {@link #previous()} has not been called
