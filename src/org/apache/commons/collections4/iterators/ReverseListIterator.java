...
/**
 * Iterates backwards through a ListCommand, starting with the last element and
 * continuing to the first. This is useful for looping around a list in reverse
 * order without needing to actually reverse the list.
 * <p>
 * The first call to <code>next()</code> will return the last element from the
 * list, and so on. The <code>hasNext()</code> method works in concert with the
 * <code>next()</code> method as expected. However, the <code>nextIndex()</code>
 * method returns the correct index in the list, thus it starts high and reduces
 * as the iteration continues. The previous methods work similarly.
 *
 * @since 3.2
...
public class ReverseListIterator<E> implements ResettableListIterator<E> {
	...
	/**
	 * Resets the iterator back to the start (which is the end of the list as this
	 * is a reversed iterator)
	 */
	@Override
	public void reset() {
		iterator = list.listIterator(list.size());
