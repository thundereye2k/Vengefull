...
abstract class AbstractPatriciaTrie<K, V> extends AbstractBitwiseTrie<K, V> {
	...
	/**
	 * The number of times this {@link Trie} has been modified. It's used to detect
	 * concurrent modifications and fail-fast the {@link Iterator}s.
	 */
	...
	/**
	 * Constructs a new {@link org.apache.commons.collections4.Trie Trie} using the
	 * given {@link KeyAnalyzer} and initializes the
	 * {@link org.apache.commons.collections4.Trie Trie} with the values from the
	 * provided {@link Map}.
	 */
	...
	/**
	 * Returns the {@link Entry} whose key is closest in a bitwise XOR metric to the
	 ...
	 * </ol>
	 *
	 * If the {@link Trie} contained 'H' and 'L', a lookup of 'D' would return 'L',
	 * because the XOR distance between D &amp; L is smaller than the XOR distance
	 * between D &amp; H.
	 *
	 * @param key
	 *            the key to use in the search
	 * @return the {@link Entry} whose key is closest in a bitwise XOR metric to the
	 *         provided key
	 */
	 ...
	 * This is NOT lexicographic closeness!
	 *
	 ...
	 * {@link #selectR(TrieEntry, int, Object, int, Cursor, Reference)} method but
	 * without its overhead because we're selecting only one best matching Entry
	 * from the {@link Trie}.
	 */
	private boolean selectR(final TrieEntry<K, V> h, final int bitIndex, final K key, final int lengthInBits,
	...
	/**
	 * Returns the nearest entry for a given key. This is useful for finding knowing
	 * if a given key exists (and finding the value for it), or for inserting the
	 * key.
	 *
	 * The actual get implementation. This is very similar to selectR but with the
	 * exception that it might return the root Entry even if it's empty.
	 */
	 ...
	 *
	 * If it's an external Entry then just remove it. This is very easy and straight
	 * forward.
	 */
	 ...
	 *
	 * If it's an internal Entry then "good luck" with understanding this code. The
	 * Idea is essentially that Entry p takes Entry h's place in the trie which
	...
	/**
	 * Returns the entry lexicographically after the given entry. If the given entry
	 * is null, returns the first node.
	 */
	 ...
	 *
	 * 1) Scan all the way to the left. a) If we already started from this node last
	 * time, proceed to Step 2. b) If a valid uplink is found, use it. c) If the
	 * result is an empty node (root not set), break the scan. d) If we already
	 * returned the left node, break the scan.
	 *
	 * 2) Check the right. a) If we already returned the right node, proceed to Step
	 * 3. b) If it is a valid uplink, use it. c) Do Step 1 from the right node.
	 *
	 * 3) Back up through the parents until we encounter find a parent that we're
	 ...
	 *
	 * 6) If the right child of the parent is the parent itself, we've already found
	 * & returned the end of the Trie, so exit.
	 *
	 ...
	 * <p>
	 * This is implemented by going always to the left until we encounter a valid
	 * uplink. That uplink is the first key.
	 */
	...
	/**
	 * Returns a view of this {@link Trie} of all elements that are prefixed by the
	 * number of bits in the given Key.
	 * <p>
	 * The view that this returns is optimized to have a very efficient
	 * {@link Iterator}. The {@link SortedMap#firstKey()},
	 * {@link SortedMap#lastKey()} &amp; {@link Map#size()} methods must iterate
	 * over all possible values in order to determine the results. This information
	 * is cached until the PATRICIA {@link Trie} changes. All other methods (except
	 * {@link Iterator}) must compare the given key to the prefix to ensure that it
	 * is within the range of the view. The {@link Iterator}'s remove method must
	 * also relocate the subtree that contains the prefixes if the entry holding the
	 ...
	 *            the prefix offset
	 * @param lengthInBits
	 *            the number of significant prefix bits
	...
	/**
	 * Returns an entry strictly higher than the given key, or null if no such entry
	 * exists.
	 */
	 ...
	 *
	 * This is very similar to getR but with the difference that we stop the lookup
	 * if h.bitIndex > lengthInBits.
	 */
	TrieEntry<K, V> subtree(final K prefix, final int offsetInBits, final int lengthInBits) {
		...
		if (isBitSet(prefix, endIndexInBits - 1, endIndexInBits) != isBitSet(entry.key, lengthInBits - 1,
				lengthInBits(entry.key))) {
			return null;
		}

		// ... or there are less than 'length' equal bits
		final int bitIndex = getKeyAnalyzer().bitIndex(prefix, offsetInBits, lengthInBits, entry.key, 0,
	 ...
	 *
	 * <p>
	 * This is implemented by going always to the right until we encounter a valid
	 * uplink. That uplink is the last key.
	 */
	 ...
	 * return it. - Else, follow the right path from the predecessor's left. - If
	 * the uplink that returned us was a left uplink: - Loop back through parents
	 * until we encounter a node where node != node.parent.left. - If
	 * node.parent.left is uplink from node.parent: - If node.parent.left is not
	 * root, return it. - If it is root & root isEmpty, return null. - If it is root
	 * & root !isEmpty, return root. - If node.parent.left is not uplink from
	 * node.parent: - Follow right path for first right child from node.parent.left
	 *
	 * @param start
	 *            the start entry
	 */
	 ...
	 * is null, returns the first node.
	 *
	 * This will traverse only within the subtree. If the given node is not within
	 * the subtree, this will have undefined results.
	 */
	...
	/**
	 * A {@link Reference} allows us to return something through a Method's argument
	 * list. An alternative would be to an Array with a length of one (1) but that
	...
	protected static class TrieEntry<K, V> extends BasicEntry<K, V> {
		...
		/**
		 * Whether or not the entry is storing a key. Only the root can potentially be
		 * empty, all other nodes must have a key.
		 */
		...
		public String toString() {
			...
			if (predecessor != null) {
				if (predecessor.bitIndex == -1) {
					...
					buffer.append("predecessor=").append(predecessor.getKey()).append(" [").append(predecessor.bitIndex)
							.append("]");
				}
			}
	...
	private abstract class RangeMap extends AbstractMap<K, V> implements SortedMap<K, V> {
		 ...
		 */
		protected abstract SortedMap<K, V> createRangeMap(K fromKey, boolean fromInclusive, K toKey,
				boolean toInclusive);
	}
	...
	private class RangeEntryMap extends RangeMap {
		...
		/**
		 * Creates a {@link RangeEntryMap} with the fromKey included and the toKey
		 ...
		 */
		protected RangeEntryMap(final K fromKey, final boolean fromInclusive, final K toKey,
		...
		@Override
		protected SortedMap<K, V> createRangeMap(final K fromKey, final boolean fromInclusive, final K toKey,
				final boolean toInclusive) {
	...
	private class PrefixRangeMap extends RangeMap {
		...
		/**
		 * This method does two things. It determines the FROM and TO range of the
		 * {@link PrefixRangeMap} and the number of elements in the range. This method
		...
		/**
		 * Returns true if this {@link PrefixRangeMap}'s key is a prefix of the provided
		 * key.
		 */
		@Override
		protected boolean inRange(final K key) {
